...
// NOTE: on OSX the endpoint returned by SZMQ_LAST_ENDPOINT may be quite long,
...
int test_defaults (int send_hwm_, int msg_cnt_, const char *endpoint_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pub_socket, SZMQ_SNDHWM, &send_hwm_, sizeof (send_hwm_)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub_socket, SZMQ_SUBSCRIBE, 0, 0));
    ...
    while (send_count < msg_cnt_
           && zmq_send (pub_socket, "test message", 13, SZMQ_DONTWAIT) == 13)
        ++send_count;
    ...
    int recv_count = 0;
    char dummybuff[64];
    while (13 == zmq_recv (sub_socket, &dummybuff, 64, SZMQ_DONTWAIT)) {
        ++recv_count;
    }
...
int test_blocking (int send_hwm_, int msg_cnt_, const char *endpoint_)
{
    ...
    int wait = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pub_socket, SZMQ_XPUB_NODROP, &wait, sizeof (wait)));
    int timeout_ms = 10;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      sub_socket, SZMQ_RCVTIMEO, &timeout_ms, sizeof (timeout_ms)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub_socket, SZMQ_SUBSCRIBE, 0, 0));
    ...
    while (send_count < msg_cnt_) {
        const int rc = zmq_send (pub_socket, NULL, 0, SZMQ_DONTWAIT);
        if (rc == 0) {
            ++send_count;
        } else if (-1 == rc) {
...
void test_reset_hwm ()
{
    ...
    // Set up bind socket
    void *pub_socket = test_context_socket (SZMQ_PUB);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pub_socket, SZMQ_SNDHWM, &hwm, sizeof (hwm)));
    bind_loopback_ipv4 (pub_socket, my_endpoint, MAX_SOCKET_STRING);

    // Set up connect socket
    void *sub_socket = test_context_socket (SZMQ_SUB);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub_socket, SZMQ_RCVHWM, &hwm, sizeof (hwm)));
    ...
    while (send_count < first_count
           && zmq_send (pub_socket, NULL, 0, SZMQ_DONTWAIT) == 0)
        ++send_count;
    ...
    // Now receive all sent messages
    int recv_count = 0;
    while (0 == zmq_recv (sub_socket, NULL, 0, SZMQ_DONTWAIT)) {
        ++recv_count;
    }
...
#if !defined(SZMQ_HAVE_WINDOWS) && !defined(SZMQ_HAVE_GNU)
