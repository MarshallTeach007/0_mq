...
void setUp ()
{
    ...
    req = test_context_socket (SZMQ_REQ);

    int enabled = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (req, SZMQ_REQ_RELAXED, &enabled, sizeof (int)));

    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (req, SZMQ_REQ_CORRELATE, &enabled, sizeof (int)));
...
static void bounce (void *socket_)
{
    ...
    do {
        ...
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_msg_send (&sent_part, socket_, more ? SZMQ_SNDMORE : 0));
...
static int get_events (void *socket_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (socket_, SZMQ_EVENTS, &events, &events_size));
    return events;
}
...
void test_case_1 ()
{
    ...
    TEST_ASSERT_EQUAL_INT (SZMQ_POLLOUT, events);
...
void test_case_4 ()
{
    ...
    //  Read the expected correlated reply. As the SZMQ_REQ_CORRELATE is active,
