...
static void test_stream_to_dealer ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (stream, SZMQ_LINGER, &zero, sizeof (zero)));
    int enabled = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (stream, SZMQ_STREAM_NOTIFY, &enabled, sizeof (enabled)));
    bind_loopback_ipv4 (stream, my_endpoint, sizeof my_endpoint);

    //  We'll be using this socket as the other peer
    void *dealer = test_context_socket (SZMQ_DEALER);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer, SZMQ_LINGER, &zero, sizeof (zero)));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_send (&routing_id, stream, SZMQ_SNDMORE));
    TEST_ASSERT_EQUAL_INT (
    ...
    TEST_ASSERT_GREATER_THAN_INT (0, TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_send (
                                       &routing_id, stream, SZMQ_SNDMORE)));
    TEST_ASSERT_EQUAL_INT (
...
static void test_stream_to_stream ()
{
    ...
    void *server = test_context_socket (SZMQ_STREAM);
    int enabled = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, SZMQ_STREAM_NOTIFY, &enabled, sizeof (enabled)));
    bind_loopback_ipv4 (server, my_endpoint, sizeof my_endpoint);

    void *client = test_context_socket (SZMQ_STREAM);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_STREAM_NOTIFY, &enabled, sizeof (enabled)));
    ...
    char http_response[] = "HTTP/1.0 200 OK\r\n"
                           ...
                           "\r\n"
                           "Hello, World!";
    TEST_ASSERT_SUCCESS_ERRNO (zmq_send (server, id, id_size, SZMQ_SNDMORE));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_send (server, http_response, sizeof (http_response), SZMQ_SNDMORE));

    //  Send zero to close connection to client
    TEST_ASSERT_SUCCESS_ERRNO (zmq_send (server, id, id_size, SZMQ_SNDMORE));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_send (server, NULL, 0, SZMQ_SNDMORE));
