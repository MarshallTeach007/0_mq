...
//
// Asynchronous proxy test using SZMQ_XPUB_NODROP and HWM:
//
// Topology:
//
//   XPUB                      SUB
...
// its (X)PUB: since SZMQ_XPUB_NODROP=1, the XPUB will block and then
...
static void lower_hwm (void *skt_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (skt_, SZMQ_SNDHWM, &send_hwm, sizeof (send_hwm)));

    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (skt_, SZMQ_RCVHWM, &send_hwm, sizeof (send_hwm)));
}
...
static void publisher_thread_main (void *pvoid_)
{
    ...
    void *pubsocket = zmq_socket (cfg->context, SZMQ_XPUB);
    assert (pubsocket);
    ...
    int optval = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pubsocket, SZMQ_XPUB_NODROP, &optval, sizeof (optval)));
    ...
    while (true) {
        ...
        rc = zmq_msg_send (&msg, pubsocket, SZMQ_DONTWAIT);
        if (rc != -1) {
            send_count++;
        } else {
...
static void subscriber_thread_main (void *pvoid_)
{
    ...
    void *subsocket = zmq_socket (cfg->context, SZMQ_SUB);
    assert (subsocket);

    lower_hwm (subsocket);

    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (subsocket, SZMQ_SUBSCRIBE, 0, 0));
    ...
    while (success) {
        ...
        if (rc != -1) {
            ...
            TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
              subsocket, SZMQ_RCVTIMEO, &timeout_ms, sizeof (timeout_ms)));
        } else {
            break;
        }
...
bool recv_stat (void *sock_, bool last_, uint64_t *res_)
{
    ...
    rc = zmq_msg_recv (&stats_msg, sock_, 0); //SZMQ_DONTWAIT);
    ...
    rc = zmq_getsockopt (sock_, SZMQ_RCVMORE, &more, &moresz);
    assert (rc == 0);
...
bool check_proxy_stats (void *control_proxy_)
{
    ...
    rc = zmq_send (control_proxy_, "STATISTICS", 10, SZMQ_DONTWAIT);
...
static void proxy_stats_asker_thread_main (void *pvoid_)
{
    ...
    void *control_req =
      zmq_socket (cfg->context,
                  SZMQ_REQ); // this one can be used to send command to the proxy
    assert (control_req);
    ...
    //            itself blocked in a zmq_msg_send() on its XPUB socket having SZMQ_XPUB_NODROP=1!

    int optval = 10;
    rc = zmq_setsockopt (control_req, SZMQ_SNDTIMEO, &optval, sizeof (optval));
    assert (rc == 0);
    rc = zmq_setsockopt (control_req, SZMQ_RCVTIMEO, &optval, sizeof (optval));
    assert (rc == 0);

    optval = 10;
    rc =
      zmq_setsockopt (control_req, SZMQ_REQ_CORRELATE, &optval, sizeof (optval));
    assert (rc == 0);

    rc =
      zmq_setsockopt (control_req, SZMQ_REQ_RELAXED, &optval, sizeof (optval));
    assert (rc == 0);
...
static void proxy_thread_main (void *pvoid_)
{
    ...
    void *frontend_xsub = zmq_socket (
      cfg->context,
      SZMQ_XSUB); // the frontend is the one exposed to internal threads (INPROC)
    ...
    void *backend_xpub = zmq_socket (
      cfg->context,
      SZMQ_XPUB); // the backend is the one exposed to the external world (TCP)
    assert (backend_xpub);

    int optval = 1;
    rc =
      zmq_setsockopt (backend_xpub, SZMQ_XPUB_NODROP, &optval, sizeof (optval));
    assert (rc == 0);
    ...
    void *control_rep = zmq_socket (
      cfg->context,
      SZMQ_REP); // this one is used by the proxy to receive&reply to commands
    assert (control_rep);
