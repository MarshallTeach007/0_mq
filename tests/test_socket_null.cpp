...
{
    TEST_ASSERT_NULL (zmq_socket (NULL, SZMQ_PAIR));
...
void test_zmq_setsockopt_null_socket ()
{
    ...
    int rc = zmq_setsockopt (NULL, SZMQ_SNDHWM, &hwm, hwm_size);
    TEST_ASSERT_EQUAL_INT (-1, rc);
    TEST_ASSERT_EQUAL_INT (ENOTSOCK, errno); // TODO use EINVAL instead?
}
...
void test_zmq_getsockopt_null_socket ()
{
    int hwm;
    size_t hwm_size = sizeof hwm;
    int rc = zmq_getsockopt (NULL, SZMQ_SNDHWM, &hwm, &hwm_size);
    TEST_ASSERT_EQUAL_INT (-1, rc);
    TEST_ASSERT_EQUAL_INT (ENOTSOCK, errno); // TODO use EINVAL instead?
}
...
{
    int rc = zmq_socket_monitor (NULL, "inproc://monitor", SZMQ_EVENT_ALL);
    TEST_ASSERT_EQUAL_INT (-1, rc);
    TEST_ASSERT_EQUAL_INT (ENOTSOCK, errno); // TODO use EINVAL instead?
}
