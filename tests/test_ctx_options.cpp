...
void test_ctx_thread_opts ()
{
    ...
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL, zmq_ctx_set (get_test_context (), SZMQ_THREAD_SCHED_POLICY,
                           SZMQ_THREAD_SCHED_POLICY_DFLT));
    TEST_ASSERT_FAILURE_ERRNO (EINVAL, zmq_ctx_set (get_test_context (),
                                                    SZMQ_THREAD_PRIORITY,
                                                    SZMQ_THREAD_PRIORITY_DFLT));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_ctx_set (get_test_context (), SZMQ_THREAD_SCHED_POLICY, TEST_POLICY));
    TEST_ASSERT_EQUAL_INT (
      TEST_POLICY, zmq_ctx_get (get_test_context (), SZMQ_THREAD_SCHED_POLICY));
    ...
    if (is_allowed_to_raise_priority ()) {
        TEST_ASSERT_SUCCESS_ERRNO (zmq_ctx_set (
          get_test_context (), SZMQ_THREAD_PRIORITY,
    ...
    for (unsigned int idx = 0; idx < sizeof (cpus_add) / sizeof (cpus_add[0]);
         idx++) {
        TEST_ASSERT_SUCCESS_ERRNO (zmq_ctx_set (
          get_test_context (), SZMQ_THREAD_AFFINITY_CPU_ADD, cpus_add[idx]));
    }
    ...
    for (unsigned int idx = 0;
        ...
        TEST_ASSERT_SUCCESS_ERRNO (zmq_ctx_set (get_test_context (),
                                                SZMQ_THREAD_AFFINITY_CPU_REMOVE,
                                                cpus_remove[idx]));
    }
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_ctx_set (get_test_context (), SZMQ_THREAD_NAME_PREFIX, 1234));
    TEST_ASSERT_EQUAL_INT (
      1234, zmq_ctx_get (get_test_context (), SZMQ_THREAD_NAME_PREFIX));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_ctx_set_ext (get_test_context (), SZMQ_THREAD_NAME_PREFIX, prefix,
                       sizeof (prefix) / sizeof (char)));

    char buf[16];
    size_t buflen = sizeof (buf) / sizeof (char);
    zmq_ctx_get_ext (get_test_context (), SZMQ_THREAD_NAME_PREFIX, buf, &buflen);
...
{
#ifdef SZMQ_ZERO_COPY_RECV
    int zero_copy;
    // Default value is 1.
    zero_copy = zmq_ctx_get (get_test_context (), SZMQ_ZERO_COPY_RECV);
    TEST_ASSERT_EQUAL_INT (1, zero_copy);

    // Test we can set it to 0.
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_ctx_set (get_test_context (), SZMQ_ZERO_COPY_RECV, 0));
    zero_copy = zmq_ctx_get (get_test_context (), SZMQ_ZERO_COPY_RECV);
    ...
    void *pull = zmq_socket (get_test_context (), SZMQ_PULL);
    char endpoint[MAX_SOCKET_STRING];
    bind_loopback_ipv4 (pull, endpoint, sizeof endpoint);

    void *push = zmq_socket (get_test_context (), SZMQ_PUSH);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_ctx_set (get_test_context (), SZMQ_ZERO_COPY_RECV, 1));
    TEST_ASSERT_EQUAL_INT (
      1, zmq_ctx_get (get_test_context (), SZMQ_ZERO_COPY_RECV));
#endif
}

void test_ctx_option_max_sockets ()
{
    TEST_ASSERT_EQUAL_INT (SZMQ_MAX_SOCKETS_DFLT,
                           zmq_ctx_get (get_test_context (), SZMQ_MAX_SOCKETS));
}

void test_ctx_option_socket_limit ()
{
#if defined(SZMQ_USE_SELECT)
    TEST_ASSERT_EQUAL_INT (FD_SETSIZE - 1, zmq_ctx_get (ctx, SZMQ_SOCKET_LIMIT));
#elif defined(SZMQ_USE_POLL) || defined(SZMQ_USE_EPOLL)                          \
  || defined(SZMQ_USE_DEVPOLL) || defined(SZMQ_USE_KQUEUE)
    TEST_ASSERT_EQUAL_INT (65535, zmq_ctx_get (ctx, SZMQ_SOCKET_LIMIT));
#endif
}

void test_ctx_option_io_threads ()
{
    TEST_ASSERT_EQUAL_INT (SZMQ_IO_THREADS_DFLT,
                           zmq_ctx_get (get_test_context (), SZMQ_IO_THREADS));
}

void test_ctx_option_ipv6 ()
{
    TEST_ASSERT_EQUAL_INT (0, zmq_ctx_get (get_test_context (), SZMQ_IPV6));
}

void test_ctx_option_msg_t_size ()
{
#if defined(SZMQ_MSG_T_SIZE)
    TEST_ASSERT_EQUAL_INT (sizeof (zmq_msg_t),
                           zmq_ctx_get (get_test_context (), SZMQ_MSG_T_SIZE));
#endif
}
...
{
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_ctx_set (get_test_context (), SZMQ_IPV6, true));
    TEST_ASSERT_EQUAL_INT (1, zmq_ctx_get (get_test_context (), SZMQ_IPV6));
}
...
void test_ctx_option_blocky ()
{
    ...
    void *router = test_context_socket (SZMQ_ROUTER);
    int value;
    size_t optsize = sizeof (int);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (router, SZMQ_IPV6, &value, &optsize));
    TEST_ASSERT_EQUAL_INT (1, value);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (router, SZMQ_LINGER, &value, &optsize));
    TEST_ASSERT_EQUAL_INT (-1, value);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_ctx_set (get_test_context (), SZMQ_BLOCKY, false));
    TEST_ASSERT_EQUAL_INT (0, TEST_ASSERT_SUCCESS_ERRNO ((zmq_ctx_get (
                                get_test_context (), SZMQ_BLOCKY))));
    router = test_context_socket (SZMQ_ROUTER);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (router, SZMQ_LINGER, &value, &optsize));
