...
void test_timeo ()
{
    ...
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN,
                               zmq_recv (frontend, buffer, 32, SZMQ_DONTWAIT));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (frontend, SZMQ_RCVTIMEO, &timeout, sizeof (int)));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (backend, SZMQ_SNDTIMEO, &timeout, sizeof (int)));
