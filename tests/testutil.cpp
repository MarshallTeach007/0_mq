...
{
    send_string_expect_success (socket_, bounce_content, SZMQ_SNDMORE);
...
static void recv_bounce_msg (void *socket_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (socket_, SZMQ_RCVMORE, &rcvmore, &sz));
...
static void send_bounce_msg_may_fail (void *socket_)
{
    int timeout = 250;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (socket_, SZMQ_SNDTIMEO, &timeout, sizeof (int)));
    int rc = zmq_send (socket_, bounce_content, 32, SZMQ_SNDMORE);
...
static void recv_bounce_msg_fail (void *socket_)
{
    ...
    char buffer[32];
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (socket_, SZMQ_RCVTIMEO, &timeout, sizeof (int)));
...
void s_send_seq (void *socket_, ...)
{
    ...
    while (true) {
        ...
        if (!prev) {
            TEST_ASSERT_SUCCESS_ERRNO (
              zmq_send (socket_, 0, 0, end ? 0 : SZMQ_SNDMORE));
        } else {
            TEST_ASSERT_SUCCESS_ERRNO (zmq_send (
              socket_, prev, strlen (prev) + 1, end ? 0 : SZMQ_SNDMORE));
        }
        if (end)
            break;
    }
...
void s_recv_seq (void *socket_, ...)
{
    ...
    while (true) {
        ...
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_getsockopt (socket_, SZMQ_RCVMORE, &more, &more_size));
...
{
    int linger = 0;
    int rc = zmq_setsockopt (socket_, SZMQ_LINGER, &linger, sizeof (linger));
...
#endif
#else
#if defined SZMQ_HAVE_CYGWIN
    // abort test after 121 seconds
    alarm (121);
#else
#if !defined SZMQ_DISABLE_TEST_TIMEOUT
...
{
#if defined(SZMQ_HAVE_WINDOWS) && (_WIN32_WINNT < 0x0600)
    return 0;
#else
    int rc, ipv6 = 1;
...
int is_tipc_available ()
{
    ...
    void *rep = zmq_socket (ctx, SZMQ_REP);
