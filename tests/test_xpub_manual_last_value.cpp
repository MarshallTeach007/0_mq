...
void test_basic ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pub, SZMQ_XPUB_MANUAL_LAST_VALUE, &manual, 4));
...
void test_unsubscribe_manual ()
{
    ...
    int manual = 1;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_XPUB_MANUAL_LAST_VALUE,
...
void test_xpub_proxy_unsubscribe_on_disconnect ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (xpub_proxy, SZMQ_XPUB_MANUAL_LAST_VALUE, &manual, 4));
...
void test_manual_last_value ()
{
    ...
    int hwm = 2000;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_SNDHWM, &hwm, 4));
    ...
    //  Subscribe for "A".
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub, SZMQ_SUBSCRIBE, "A", 1));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_SUBSCRIBE, "A", 1));
    send_string_expect_success (pub, "A", 0);
    recv_string_expect_success (sub, "A", 0);

    //  Subscribe for "A".
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub2, SZMQ_SUBSCRIBE, "A", 1));
    recv_array_expect_success (pub, subscription, 0);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_SUBSCRIBE, "A", 1));
    send_string_expect_success (pub, "A", 0);
