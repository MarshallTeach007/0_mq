...
void test ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (server, SZMQ_LAST_ENDPOINT, address, &addr_length));

    //  Create a client
    void *client = test_context_socket (SZMQ_CLIENT);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_HELLO_MSG, "HELLO", 5));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_HICCUP_MSG, "HICCUP", 6));
