    ...
    while (count < max_client_connect) {
        ...
        do {
            ...
            zmq_pollitem_t items[] = {
              {NULL, client, SZMQ_POLLIN, 0},
              {NULL, remote, SZMQ_POLLIN, 0},
            };
            fprintf (stderr,
            ...
            while (1) {
                ...
                int nbytes;
                for (int i = 0; i < 2; i++) {
                    if ((items[i].revents & SZMQ_POLLIN) == 0)
                        continue;
...
void *setup_push_server (char *connect_address, int connect_address_size)
{
    ...
    res = zmq_getsockopt (push, SZMQ_LAST_ENDPOINT, connect_address, &len);
    TEST_ASSERT_SUCCESS_ERRNO (res);
...
void *setup_pull_client (const char *connect_address, const char *socks_proxy)
{
    ...
    if (socks_proxy != NULL) {
        res = zmq_setsockopt (pull, SZMQ_SOCKS_PROXY, socks_proxy,
                              strlen (socks_proxy));
        TEST_ASSERT_SUCCESS_ERRNO (res);
                                   ...
                                   const char *password)
{
    ...
    res = zmq_setsockopt (pull, SZMQ_SOCKS_USERNAME, username,
                          username == NULL ? 0 : strlen (username));
    TEST_ASSERT_SUCCESS_ERRNO (res);

    res = zmq_setsockopt (pull, SZMQ_SOCKS_PASSWORD, password,
...
void test_socks_proxy_options (void)
{
    ...
    test_opt_ok ("NULL proxy", SZMQ_SOCKS_PROXY, NULL, 0, "", 0);
    test_string_opt_ok ("valid proxy", SZMQ_SOCKS_PROXY, "somehost:1080");
    // Empty value not allowed for proxy server
    test_opt_invalid ("empty proxy", SZMQ_SOCKS_PROXY, "", 0);
}
...
void test_socks_userpass_options (void)
{
    ...
    test_opt_ok ("NULL username", SZMQ_SOCKS_USERNAME, NULL, 0, "", 0);
    // Empty value is allowed for username, means no authentication
    test_string_opt_ok ("empty username", SZMQ_SOCKS_USERNAME, "");
    test_string_opt_ok ("valid username", SZMQ_SOCKS_USERNAME, "someuser");
    test_opt_ok ("255 bytes username", SZMQ_SOCKS_USERNAME, buffer, 255, buffer,
                 255);
    test_opt_invalid ("too long username", SZMQ_SOCKS_USERNAME, buffer, 256);

    // NULL is equivalent to not-set or ""
    test_opt_ok ("NULL password", SZMQ_SOCKS_PASSWORD, NULL, 0, "", 0);
    // Empty value allowed for password
    test_string_opt_ok ("empty password", SZMQ_SOCKS_PASSWORD, "");
    test_string_opt_ok ("valid password", SZMQ_SOCKS_PASSWORD, "someuser");
    test_opt_ok ("255 bytes password", SZMQ_SOCKS_PASSWORD, buffer, 255, buffer,
                 255);
    test_opt_invalid ("too long password", SZMQ_SOCKS_PASSWORD, buffer, 256);
#else
