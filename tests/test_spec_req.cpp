...
{
    void *req = test_context_socket (SZMQ_REQ);

    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (req, bind_address_));
    size_t len = MAX_SOCKET_STRING;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (req, SZMQ_LAST_ENDPOINT, connect_address, &len));
...
void test_req_only_listens_to_current_peer (const char *bind_address_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (req, SZMQ_ROUTING_ID, "A", 2));
    ...
    for (size_t i = 0; i < services; ++i) {
        router[i] = test_context_socket (SZMQ_ROUTER);

        int enabled = 1;
        TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
          router[i], SZMQ_ROUTER_MANDATORY, &enabled, sizeof (enabled)));
...
void test_req_message_format (const char *bind_address_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (router, SZMQ_RCVMORE, &more, &more_size));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_msg_send (&peer_id_msg, router, SZMQ_SNDMORE));
