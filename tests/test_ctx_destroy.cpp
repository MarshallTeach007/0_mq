...
void test_ctx_destroy ()
{
    ...
    void *socket = zmq_socket (ctx, SZMQ_PULL);
...
void test_ctx_shutdown_socket_opened_after ()
{
    ...
    TEST_ASSERT_NULL (zmq_socket (ctx, SZMQ_PULL));
...
void test_poller_exists_with_socket_on_zmq_ctx_term_thread_safe_socket ()
{
    ...
    test_poller_exists_with_socket_on_zmq_ctx_term (SZMQ_CLIENT);
#else
...
{
    test_poller_exists_with_socket_on_zmq_ctx_term (SZMQ_DEALER);
}
