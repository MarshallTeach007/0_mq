...
#if defined SZMQ_CUSTOM_PLATFORM_HPP
#include "platform.hpp"
#else
#include "../src/platform.hpp"
#endif

#ifndef SZMQ_USE_TWEETNACL
#define SZMQ_USE_TWEETNACL
#endif
#ifdef SZMQ_USE_LIBSODIUM
                    ...
                    char *client_secret_)
{
    ...
    int handshake_failed_encryption_event_count =
      expect_monitor_event_multiple (server_mon_,
                                     ...
                                     SZMQ_PROTOCOL_ERROR_ZMTP_CRYPTOGRAPHIC);
    ...
    fprintf (stderr,
             "count of "
             "SZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL/"
             "SZMQ_PROTOCOL_ERROR_ZMTP_CRYPTOGRAPHIC events: %i\n",
...
void test_curve_security_with_valid_credentials ()
{
    ...
    assert (event == SZMQ_EVENT_HANDSHAKE_SUCCEEDED);
...
void test_curve_security_with_bogus_client_credentials ()
{
    ...
    expect_new_client_curve_bounce_fail (
      ...
      NULL, SZMQ_EVENT_HANDSHAKE_FAILED_AUTH, 400);

    int server_event_count = 0;
    server_event_count = expect_monitor_event_multiple (
      server_mon, SZMQ_EVENT_HANDSHAKE_FAILED_AUTH, 400);
                                     ...
                                     void *server_mon_)
{
    ...
    expect_monitor_event_multiple (server_mon_,
                                   SZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL,
                                   SZMQ_PROTOCOL_ERROR_ZMTP_MECHANISM_MISMATCH);
...
void test_curve_security_invalid_hello_wrong_length ()
{
    ...
    expect_monitor_event_multiple (
      server_mon, SZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL,
      SZMQ_PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_HELLO);
...
void test_curve_security_invalid_hello_command_name ()
{
    ...
    expect_monitor_event_multiple (server_mon,
                                   SZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL,
                                   SZMQ_PROTOCOL_ERROR_ZMTP_UNEXPECTED_COMMAND);
...
void test_curve_security_invalid_initiate_wrong_length ()
{
    ...
    expect_monitor_event_multiple (
      server_mon, SZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL,
      SZMQ_PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_INITIATE);
...
void test_curve_security_invalid_keysize (void *ctx_)
{
    ...
    void *client = zmq_socket (ctx_, SZMQ_DEALER);
    TEST_ASSERT_NOT_NULL (client);
    errno = 0;
    int rc =
      zmq_setsockopt (client, SZMQ_CURVE_SERVERKEY, valid_server_public, 123);
    assert (rc == -1 && errno == EINVAL);
    errno = 0;
    rc = zmq_setsockopt (client, SZMQ_CURVE_PUBLICKEY, valid_client_public, 123);
    assert (rc == -1 && errno == EINVAL);
    errno = 0;
    rc = zmq_setsockopt (client, SZMQ_CURVE_SECRETKEY, valid_client_secret, 123);
    assert (rc == -1 && errno == EINVAL);
