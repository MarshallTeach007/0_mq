...
void test_probe_router_router ()
{
    ...
    void *client = test_context_socket (SZMQ_ROUTER);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (client, SZMQ_ROUTING_ID, "X", 1));
    int probe = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_PROBE_ROUTER, &probe, sizeof (probe)));
    ...
    send_string_expect_success (server, "X", SZMQ_SNDMORE);
