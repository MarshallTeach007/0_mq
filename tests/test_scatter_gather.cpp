...
{
    void *scatter = test_context_socket (SZMQ_SCATTER);
    void *gather = test_context_socket (SZMQ_GATHER);
    ...
    TEST_ASSERT_FAILURE_ERRNO (EINVAL,
                               zmq_send_const (scatter, "1", 1, SZMQ_SNDMORE));
...
void test_scatter_gather ()
{
    void *scatter = test_context_socket (SZMQ_SCATTER);
    void *gather = test_context_socket (SZMQ_GATHER);
    void *gather2 = test_context_socket (SZMQ_GATHER);
