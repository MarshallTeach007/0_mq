...
void test_push_round_robin_out (const char *bind_address_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (push, SZMQ_LAST_ENDPOINT, connect_address, &len));

    const size_t services = 5;
    void *pulls[services];
    for (size_t peer = 0; peer < services; ++peer) {
        pulls[peer] = test_context_socket (SZMQ_PULL);
...
{
    void *pull = test_context_socket (SZMQ_PULL);

    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (pull, bind_address_));
    size_t len = MAX_SOCKET_STRING;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (pull, SZMQ_LAST_ENDPOINT, connect_address, &len));

    const unsigned char services = 5;
    void *pushs[services];
    for (unsigned char peer = 0; peer < services; ++peer) {
        pushs[peer] = test_context_socket (SZMQ_PUSH);
...
{
    void *sc = test_context_socket (SZMQ_PUSH);

    int timeout = 250;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sc, SZMQ_SNDTIMEO, &timeout, sizeof (timeout)));

    TEST_ASSERT_FAILURE_ERRNO (EAGAIN, zmq_send (sc, 0, 0, SZMQ_DONTWAIT));
...
{
    void *a = test_context_socket (SZMQ_PUSH);

    int hwm = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (a, SZMQ_SNDHWM, &hwm, sizeof (hwm)));

    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (a, bind_address_));
    size_t len = MAX_SOCKET_STRING;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (a, SZMQ_LAST_ENDPOINT, connect_address, &len));

    void *b = test_context_socket (SZMQ_PULL);

    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (b, SZMQ_RCVHWM, &hwm, sizeof (hwm)));
    ...
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN, zmq_send (a, 0, 0, SZMQ_DONTWAIT));
    ...
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN, zmq_msg_recv (&msg, b, SZMQ_DONTWAIT));
                                      ...
                                      const bool block_)
{
    ...
    void *push = test_context_socket (SZMQ_PUSH);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (push, SZMQ_LINGER, &linger, sizeof (linger)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (push, SZMQ_SNDHWM, &hwm, sizeof (hwm)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (push, bind_address_));
    size_t addr_len = MAX_SOCKET_STRING;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (push, SZMQ_LAST_ENDPOINT, connect_address, &addr_len));

    void *pull = test_context_socket (SZMQ_PULL);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pull, SZMQ_LINGER, &linger, sizeof (linger)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pull, SZMQ_RCVHWM, &hwm, sizeof (hwm)));
    ...
    send_string_expect_success (push, "0", SZMQ_SNDMORE);
    send_string_expect_success (push, "0", SZMQ_SNDMORE);
    ...
    send_string_expect_success (push, "2", SZMQ_SNDMORE);
    send_string_expect_success (push, "2", SZMQ_SNDMORE);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_init_size (&msg_data, len));
    ...
    zmq_getsockopt (push, SZMQ_EVENTS, &zmq_events, &zmq_events_size);
    zmq_getsockopt (pull, SZMQ_EVENTS, &zmq_events, &zmq_events_size);
    msleep (SETTLE_TIME);
    zmq_getsockopt (push, SZMQ_EVENTS, &zmq_events, &zmq_events_size);
    zmq_getsockopt (pull, SZMQ_EVENTS, &zmq_events, &zmq_events_size);
    ...
    send_string_expect_success (push, "3", SZMQ_SNDMORE);
    send_string_expect_success (push, "3", SZMQ_SNDMORE);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_init_size (&msg_data, len));
    memset (zmq_msg_data (&msg_data), 'c', len);
    if (block_) {
        TEST_ASSERT_EQUAL_INT (len,
                               zmq_msg_send (&msg_data, push, SZMQ_SNDMORE));
    } else {
        rc = zmq_msg_send (&msg_data, push, SZMQ_SNDMORE | SZMQ_DONTWAIT);
    ...
    send_string_expect_success (push, "5", SZMQ_SNDMORE);
    send_string_expect_success (push, "5", SZMQ_SNDMORE);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_init_size (&msg_data, len));
