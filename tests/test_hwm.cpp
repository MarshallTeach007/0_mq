...
void test_defaults ()
{
    ...
    int recv_count = 0;
    while (zmq_recv (bind_socket, NULL, 0, SZMQ_DONTWAIT) == 0)
        ++recv_count;
...
int count_msg (int send_hwm_, int recv_hwm_, TestType test_type_)
{
    ...
    if (test_type_ == BIND_FIRST) {
        // Set up bind socket
        bind_socket = test_context_socket (SZMQ_PULL);
        TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
          bind_socket, SZMQ_RCVHWM, &recv_hwm_, sizeof (recv_hwm_)));
        TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (bind_socket, "inproc://a"));

        // Set up connect socket
        connect_socket = test_context_socket (SZMQ_PUSH);
        TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
          connect_socket, SZMQ_SNDHWM, &send_hwm_, sizeof (send_hwm_)));
        TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (connect_socket, "inproc://a"));
...
int test_inproc_connect_and_close_first (int send_hwm_, int recv_hwm_)
{
    // Set up connect socket
    void *connect_socket = test_context_socket (SZMQ_PUSH);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (connect_socket, SZMQ_SNDHWM,
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (bind_socket, SZMQ_RCVHWM, &recv_hwm_, sizeof (recv_hwm_)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (bind_socket, "inproc://a"));
...
{
    // Set up bind socket
    void *bind_socket = test_context_socket (SZMQ_PUSH);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (bind_socket, SZMQ_SNDHWM, &send_hwm_, sizeof (send_hwm_)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (bind_socket, "inproc://a"));

    // Send until we block
    int send_count = 0;
    while (send_count < MAX_SENDS
           && zmq_send (bind_socket, NULL, 0, SZMQ_DONTWAIT) == 0)
        ++send_count;
    ...
    // Set up connect socket
    void *connect_socket = test_context_socket (SZMQ_PULL);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (connect_socket, SZMQ_RCVHWM, &recv_hwm, sizeof (recv_hwm)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (connect_socket, "inproc://a"));

    // Now receive all sent messages
    int recv_count = 0;
    while (zmq_recv (connect_socket, NULL, 0, SZMQ_DONTWAIT) == 0)
        ++recv_count;
