...
void test ()
{
    ...
    int wait = 1;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_XPUB_NODROP, &wait, 4));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub, SZMQ_SUBSCRIBE, "", 0));
    ...
    do {
        ...
        if (recv_count == 1) {
            ...
            TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
              sub, SZMQ_RCVTIMEO, &sub_rcvtimeo, sizeof (sub_rcvtimeo)));
        }
    ...
    int timeout = 0;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_SNDTIMEO, &timeout, 4));
    ...
    if (send_count > 0) {
        ...
        while (zmq_recv (sub, NULL, 0, SZMQ_DONTWAIT) == 0)
            recv_count++;
    }
