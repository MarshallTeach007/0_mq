...
static void zap_handler (void *handler_)
{
    ...
    while (true) {
        ...
        send_string_expect_success (handler_, version, SZMQ_SNDMORE);
        send_string_expect_success (handler_, sequence, SZMQ_SNDMORE);

        if (!zap_deny_all) {
            send_string_expect_success (handler_, "200", SZMQ_SNDMORE);
            send_string_expect_success (handler_, "OK", SZMQ_SNDMORE);
            send_string_expect_success (handler_, "anonymous", SZMQ_SNDMORE);
            send_string_expect_success (handler_, "", 0);
            //fprintf (stderr, "ALLOW %s\n", principal);
        } else {
            send_string_expect_success (handler_, "400", SZMQ_SNDMORE);
            send_string_expect_success (handler_, "Denied", SZMQ_SNDMORE);
            send_string_expect_success (handler_, "", SZMQ_SNDMORE);
            send_string_expect_success (handler_, "", 0);
...
void setUp ()
{
    ...
    server = test_context_socket (SZMQ_DEALER);
    int as_server = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, SZMQ_GSSAPI_SERVER, &as_server, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, SZMQ_GSSAPI_PRINCIPAL, name, strlen (name) + 1));
    int name_type = SZMQ_GSSAPI_NT_HOSTBASED;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      server, SZMQ_GSSAPI_PRINCIPAL_NAMETYPE, &name_type, sizeof (name_type)));
    bind_loopback_ipv4 (server, my_endpoint, sizeof my_endpoint);

    //  Monitor handshake events on the server
    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_monitor (
      server, "inproc://monitor-server",
      SZMQ_EVENT_HANDSHAKE_SUCCEEDED | SZMQ_EVENT_HANDSHAKE_FAILED_AUTH
        | SZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL));

    //  Create socket for collecting monitor events
    server_mon = test_context_socket (SZMQ_PAIR);
...
void test_valid_creds ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      client, SZMQ_GSSAPI_SERVICE_PRINCIPAL, name, strlen (name) + 1));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_GSSAPI_PRINCIPAL, name, strlen (name) + 1));
    int name_type = SZMQ_GSSAPI_NT_HOSTBASED;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      client, SZMQ_GSSAPI_PRINCIPAL_NAMETYPE, &name_type, sizeof (name_type)));
    ...
    TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_HANDSHAKE_SUCCEEDED, event);
}
...
void test_unauth_creds ()
{
    ...
    int event = get_monitor_event (server_mon, NULL, NULL);
    TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_HANDSHAKE_FAILED_AUTH, event);
}
...
void test_null_creds ()
{
    ...
    TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL, event);
    TEST_ASSERT_EQUAL_INT (SZMQ_PROTOCOL_ERROR_ZMTP_MECHANISM_MISMATCH, error);
}
...
void test_plain_creds ()
{
    void *client = test_context_socket (SZMQ_DEALER);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_PLAIN_USERNAME, "admin", 5));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_PLAIN_PASSWORD, "password", 8));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (client, my_endpoint));
    expect_bounce_fail (server, client);
    test_context_socket_close_zero_linger (client);
}
...
void test_vanilla_socket ()
{
    ...
    int timeout = 250;
    zmq_setsockopt (server, SZMQ_RCVTIMEO, &timeout, sizeof (timeout));
