...
{
    void *client = test_context_socket (SZMQ_DEALER);

    //  Socket monitoring only works over inproc://
    TEST_ASSERT_FAILURE_ERRNO (
      EPROTONOSUPPORT, zmq_socket_monitor (client, "tcp://127.0.0.1:*", 0));

#ifdef SZMQ_EVENT_PIPES_STATS
...
void test_monitor_basic ()
{
    ...
    void *client = test_context_socket (SZMQ_DEALER);
    void *server = test_context_socket (SZMQ_DEALER);

    //  Monitor all events on client and server sockets
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_socket_monitor (client, "inproc://monitor-client", SZMQ_EVENT_ALL));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_socket_monitor (server, "inproc://monitor-server", SZMQ_EVENT_ALL));

    //  Create two sockets for collecting monitor events
    void *client_mon = test_context_socket (SZMQ_PAIR);
    void *server_mon = test_context_socket (SZMQ_PAIR);
    ...
    if (event == SZMQ_EVENT_CONNECT_DELAYED)
        event = get_monitor_event (client_mon, NULL, NULL);
    TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_CONNECTED, event);
    expect_monitor_event (client_mon, SZMQ_EVENT_HANDSHAKE_SUCCEEDED);
    event = get_monitor_event (client_mon, NULL, NULL);
    if (event == SZMQ_EVENT_DISCONNECTED) {
        expect_monitor_event (client_mon, SZMQ_EVENT_CONNECT_RETRIED);
        expect_monitor_event (client_mon, SZMQ_EVENT_MONITOR_STOPPED);
    } else
        TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_MONITOR_STOPPED, event);

    //  This is the flow of server events
    expect_monitor_event (server_mon, SZMQ_EVENT_LISTENING);
    expect_monitor_event (server_mon, SZMQ_EVENT_ACCEPTED);
    expect_monitor_event (server_mon, SZMQ_EVENT_HANDSHAKE_SUCCEEDED);
    event = get_monitor_event (server_mon, NULL, NULL);
    //  Sometimes the server sees the client closing before it gets closed.
    if (event != SZMQ_EVENT_DISCONNECTED) {
        TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_CLOSED, event);
        event = get_monitor_event (server_mon, NULL, NULL);
    }
    if (event != SZMQ_EVENT_DISCONNECTED) {
        TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_MONITOR_STOPPED, event);
    ...
    while (event != SZMQ_EVENT_MONITOR_STOPPED) {
        event = get_monitor_event (server_mon, NULL, NULL);
    }
...
#if (defined SZMQ_CURRENT_EVENT_VERSION && SZMQ_CURRENT_EVENT_VERSION >= 2)      \
  || (defined SZMQ_CURRENT_EVENT_VERSION                                        \
      && SZMQ_CURRENT_EVENT_VERSION_DRAFT >= 2)
void test_monitor_versioned_invalid_socket_type ()
{
    void *client = test_context_socket (SZMQ_DEALER);

    //  Socket monitoring only works with SZMQ_PAIR, SZMQ_PUB and SZMQ_PUSH.
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL, zmq_socket_monitor_versioned (
                client, "inproc://invalid-socket-type", 0, 2, SZMQ_CLIENT));
                                   ...
                                   int type_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_monitor_versioned (
      client, client_mon_endpoint, SZMQ_EVENT_ALL_V2, 2, type_));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_monitor_versioned (
      server, server_mon_endpoint, SZMQ_EVENT_ALL_V2, 2, type_));

    //  Choose the appropriate consumer socket type.
    int mon_type = SZMQ_PAIR;
    switch (type_) {
        case SZMQ_PAIR:
            mon_type = SZMQ_PAIR;
            break;
    ...
    if (type_ == SZMQ_PUB) {
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_setsockopt (client_mon, SZMQ_SUBSCRIBE, "", 0));
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_setsockopt (server_mon, SZMQ_SUBSCRIBE, "", 0));
    }
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (server, SZMQ_IPV6, &ipv6, &ipv6_size));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_IPV6, &ipv6, sizeof (int)));
    ...
    TEST_ASSERT_EQUAL (SZMQ_EVENT_CONNECTED, event);
    ...
    expect_monitor_event_v2 (client_mon, SZMQ_EVENT_HANDSHAKE_SUCCEEDED,
                             client_local_address, client_remote_address);
    event = get_monitor_event_v2 (client_mon, NULL, NULL, NULL);
    if (event == SZMQ_EVENT_DISCONNECTED) {
        expect_monitor_event_v2 (client_mon, SZMQ_EVENT_CONNECT_RETRIED,
                                 client_local_address, client_remote_address);
        expect_monitor_event_v2 (client_mon, SZMQ_EVENT_MONITOR_STOPPED, "", "");
    } else
        TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_MONITOR_STOPPED, event);

    //  This is the flow of server events
    expect_monitor_event_v2 (server_mon, SZMQ_EVENT_LISTENING,
                             client_remote_address, "");
    expect_monitor_event_v2 (server_mon, SZMQ_EVENT_ACCEPTED,
                             client_remote_address, client_local_address);
    expect_monitor_event_v2 (server_mon, SZMQ_EVENT_HANDSHAKE_SUCCEEDED,
                             client_remote_address, client_local_address);
...
void test_monitor_versioned_basic_tcp_ipv4 ()
{
    ...
    test_monitor_versioned_basic (bind_loopback_ipv4, prefix, SZMQ_PAIR);
    test_monitor_versioned_basic (bind_loopback_ipv4, prefix, SZMQ_PUB);
    test_monitor_versioned_basic (bind_loopback_ipv4, prefix, SZMQ_PUSH);
}

void test_monitor_versioned_basic_tcp_ipv6 ()
{
    static const char prefix[] = "tcp://[::1]:";
    test_monitor_versioned_basic (bind_loopback_ipv6, prefix, SZMQ_PAIR);
    test_monitor_versioned_basic (bind_loopback_ipv6, prefix, SZMQ_PUB);
    test_monitor_versioned_basic (bind_loopback_ipv6, prefix, SZMQ_PUSH);
}

void test_monitor_versioned_basic_ipc ()
{
    static const char prefix[] = "ipc://";
    test_monitor_versioned_basic (bind_loopback_ipc, prefix, SZMQ_PAIR);
    test_monitor_versioned_basic (bind_loopback_ipc, prefix, SZMQ_PUB);
    test_monitor_versioned_basic (bind_loopback_ipc, prefix, SZMQ_PUSH);
}

void test_monitor_versioned_basic_tipc ()
{
    static const char prefix[] = "tipc://";
    test_monitor_versioned_basic (bind_loopback_tipc, prefix, SZMQ_PAIR);
    test_monitor_versioned_basic (bind_loopback_tipc, prefix, SZMQ_PUB);
    test_monitor_versioned_basic (bind_loopback_tipc, prefix, SZMQ_PUSH);
}
                                   ...
                                   const char *expected_prefix_)
{
    ...
    //  We'll monitor these two sockets
    void *push = test_context_socket (SZMQ_PUSH);

    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_monitor_versioned (
      push, "inproc://monitor-push", SZMQ_EVENT_PIPES_STATS, 2, SZMQ_PAIR));

    //  Should fail if there are no pipes to monitor
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN, zmq_socket_monitor_pipes_stats (push));

    void *push_mon = test_context_socket (SZMQ_PAIR);
    ...
    int send_hwm = 500;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (push, SZMQ_SNDHWM, &send_hwm, sizeof (send_hwm)));
    //  Set very low TCP buffers so that messages cannot be stored in-flight
    const int tcp_buffer_size = 4096;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      push, SZMQ_SNDBUF, &tcp_buffer_size, sizeof (tcp_buffer_size)));
    bind_function_ (push, server_endpoint, sizeof (server_endpoint));

    int ipv6;
    size_t ipv6_size = sizeof (ipv6);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (push, SZMQ_IPV6, &ipv6, &ipv6_size));
    for (int i = 0; i < pulls_count; ++i) {
        pulls[i] = test_context_socket (SZMQ_PULL);
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_setsockopt (pulls[i], SZMQ_IPV6, &ipv6, sizeof (int)));
        int timeout_ms = 10;
        TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
          pulls[i], SZMQ_RCVTIMEO, &timeout_ms, sizeof (timeout_ms)));
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_setsockopt (pulls[i], SZMQ_RCVHWM, &send_hwm, sizeof (send_hwm)));
        TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
          pulls[i], SZMQ_RCVBUF, &tcp_buffer_size, sizeof (tcp_buffer_size)));
    ...
    zmq_getsockopt (push, SZMQ_EVENTS, &dummy, &dummy_size);
    ...
    for (int i = 0; i < pulls_count; ++i) {
        ...
        TEST_ASSERT_EQUAL_STRING_LEN (expected_prefix_, push_remote_address,
                                      strlen (expected_prefix_));
        TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_PIPES_STATS, event);
