...
void reconnect_default ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_socket_monitor (sub, "inproc://monitor-sub", SZMQ_EVENT_ALL));
    //  Create socket for collecting monitor events
    void *sub_mon = test_context_socket (SZMQ_PAIR);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub, SZMQ_RECONNECT_IVL, &interval, sizeof (interval)));
    // connect to pub
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (sub, ENDPOINT_0));

    //  confirm that we get following events
    expect_monitor_event (sub_mon, SZMQ_EVENT_CONNECT_DELAYED);
    expect_monitor_event (sub_mon, SZMQ_EVENT_CONNECTED);
    expect_monitor_event (sub_mon, SZMQ_EVENT_HANDSHAKE_SUCCEEDED);

    // close the pub socket
    test_context_socket_close_zero_linger (pub);

    //  confirm that we get following events
    expect_monitor_event (sub_mon, SZMQ_EVENT_DISCONNECTED);
    expect_monitor_event (sub_mon, SZMQ_EVENT_CONNECT_RETRIED);

    // SZMQ_EVENT_CONNECT_RETRIED should be last event, because of timeout set above
    int event;
    char *event_address;
...
void reconnect_success ()
{
    ...
    pub = test_context_socket (SZMQ_PUB);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (pub, ENDPOINT_0));
    msleep (SETTLE_TIME);
    ...
    // SZMQ_EVENT_HANDSHAKE_SUCCEEDED should be last event
...
void reconnect_stop_on_refused ()
{
    ...
    int stopReconnectOnError = SZMQ_RECONNECT_STOP_CONN_REFUSED;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub, SZMQ_RECONNECT_STOP,
    ...
    expect_monitor_event (sub_mon, SZMQ_EVENT_CONNECT_RETRIED);
    expect_monitor_event (sub_mon, SZMQ_EVENT_CONNECT_DELAYED);
    expect_monitor_event (sub_mon, SZMQ_EVENT_CLOSED);

    // SZMQ_EVENT_CLOSED should be last event, because of SZMQ_RECONNECT_STOP set above
    int event = 0;
    char *event_address;
    int rc = get_monitor_event_with_timeout (sub_mon, &event, &event_address,
                                             2 * 1000);
...
void reconnect_stop_on_handshake_failed ()
{
    ...
    void *dummy = test_context_socket (SZMQ_STREAM);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (dummy, "tcp://127.0.0.1:0"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (dummy, SZMQ_LAST_ENDPOINT, bind_address, &addr_length));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      sub, SZMQ_HANDSHAKE_IVL, &handshakeInterval, sizeof (handshakeInterval)));
    // set option to stop reconnecting on failed handshake
    int stopReconnectOnError = SZMQ_RECONNECT_STOP_HANDSHAKE_FAILED;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub, SZMQ_RECONNECT_STOP,
                                               &stopReconnectOnError,
                                               sizeof (stopReconnectOnError)));
...
#if 1
    // SZMQ_EVENT_DISCONNECTED should be last event, because of SZMQ_RECONNECT_STOP set above
    expect_monitor_event (sub_mon, SZMQ_EVENT_CONNECT_DELAYED);
    expect_monitor_event (sub_mon, SZMQ_EVENT_CONNECTED);
    expect_monitor_event (sub_mon, SZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL);
    expect_monitor_event (sub_mon, SZMQ_EVENT_DISCONNECTED);
#else
