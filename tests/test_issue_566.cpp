...
void test_issue_566 ()
{
    ...
    void *router = zmq_socket (ctx1, SZMQ_ROUTER);
    int on = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (router, SZMQ_ROUTER_MANDATORY, &on, sizeof (on)));
    ...
    for (int cycle = 0; cycle < 100; cycle++) {
        ...
        void *dealer = zmq_socket (ctx2, SZMQ_DEALER);
        //  Leave space for NULL char from sprintf, gcc warning
        char routing_id[11];
        sprintf (routing_id, "%09d", cycle);
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_setsockopt (dealer, SZMQ_ROUTING_ID, routing_id, 10));
        int rcvtimeo = 1000;
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_setsockopt (dealer, SZMQ_RCVTIMEO, &rcvtimeo, sizeof (int)));
        ...
        for (int attempt = 0; attempt < 500; attempt++) {
            zmq_poll (NULL, 0, 2);
            int rc = zmq_send (router, routing_id, 10, SZMQ_SNDMORE);
