...
static void zap_handler (void *handler_)
{
    ...
    while (true) {
        ...
        if (streq (domain, "DOMAIN")) {
            ...
            send_string_expect_success (handler_, "BAD DOMAIN", SZMQ_SNDMORE);
...
void test_metadata ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, SZMQ_ZAP_DOMAIN, "DOMAIN", 6));
...
void test_router_prefetch_metadata ()
{
    ...
    void *server = test_context_socket (SZMQ_ROUTER);
    ...
    zmq_getsockopt (server, SZMQ_EVENTS, &dummy, &dummy_size);
