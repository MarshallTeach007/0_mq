...
void pre_allocate_sock_tcp (void *socket_, char *my_endpoint_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (socket_, SZMQ_USE_FD, &s, sizeof (s)));
}
...
{
    test_socket_pair (pre_allocate_sock_fun_, SZMQ_REP, SZMQ_REQ);
}

void test_pair (pre_allocate_sock_fun_t pre_allocate_sock_fun_)
{
    test_socket_pair (pre_allocate_sock_fun_, SZMQ_PAIR, SZMQ_PAIR);
}
...
void test_client_server (pre_allocate_sock_fun_t pre_allocate_sock_fun_)
{
    ...
    void *sb, *sc;
    setup_socket_pair (pre_allocate_sock_fun_, SZMQ_SERVER, SZMQ_CLIENT, &sb,
                       &sc);
    ...
    int rc = zmq_msg_send (&msg, sc, SZMQ_SNDMORE);
    ...
    rc = zmq_msg_send (&msg, sb, SZMQ_SNDMORE);
    // TODO which error code is expected?
    TEST_ASSERT_EQUAL_INT (-1, rc);
...
void pre_allocate_sock_ipc (void *sb_, char *my_endpoint_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sb_, SZMQ_USE_FD, &s, sizeof (s)));
