...
void test_xsub_verbose_unsubscribe ()
{
    ...
    // set option SZMQ_XPUB_VERBOSER to get all messages
    int xbup_verboser = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pub, SZMQ_XPUB_VERBOSER, &xbup_verboser, sizeof (int)));
    ...
    // set option SZMQ_XSUB_VERBOSE_UNSUBSCRIBE to get duplicate unsubscribes
    int xsub_verbose = 1;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      sub, SZMQ_XSUB_VERBOSE_UNSUBSCRIBE, &xsub_verbose, sizeof (int)));

    // unsubscribe from topic A, does not exist yet
    send_array_expect_success (sub, unsubscribe_a_msg, 0);

    // does not exist, but with SZMQ_XSUB_VERBOSE_UNSUBSCRIBE set it will be forwarded anyway
    recv_array_expect_success (pub, unsubscribe_a_msg, 0);
