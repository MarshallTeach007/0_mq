...
{
    LIBSZMQ_UNUSED (server_);
    LIBSZMQ_UNUSED (server_secret_);
}

void socket_config_null_server (void *server_, void *server_secret_)
{
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      server_, SZMQ_ZAP_DOMAIN, test_zap_domain, strlen (test_zap_domain)));
#ifdef SZMQ_ZAP_ENFORCE_DOMAIN
    int required = server_secret_ ? *static_cast<int *> (server_secret_) : 0;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (server_, SZMQ_ZAP_ENFORCE_DOMAIN,
                                               &required, sizeof (int)));
#else
    LIBSZMQ_UNUSED (server_secret_);
#endif
...
void socket_config_plain_client (void *server_, void *server_secret_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server_, SZMQ_PLAIN_PASSWORD, test_plain_password, 8));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server_, SZMQ_PLAIN_USERNAME, test_plain_username, 8));
}
...
void socket_config_plain_server (void *server_, void *server_secret_)
{
    ...
    int as_server = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server_, SZMQ_PLAIN_SERVER, &as_server, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      server_, SZMQ_ZAP_DOMAIN, test_zap_domain, strlen (test_zap_domain)));
}
...
void socket_config_curve_server (void *server_, void *server_secret_)
{
    int as_server = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server_, SZMQ_CURVE_SERVER, &as_server, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server_, SZMQ_CURVE_SECRETKEY, server_secret_, 41));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      server_, SZMQ_ZAP_DOMAIN, test_zap_domain, strlen (test_zap_domain)));
...
void socket_config_curve_client (void *client_, void *data_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      client_, SZMQ_CURVE_SERVERKEY, curve_client_data->server_public, 41));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      client_, SZMQ_CURVE_PUBLICKEY, curve_client_data->client_public, 41));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      client_, SZMQ_CURVE_SECRETKEY, curve_client_data->client_secret, 41));
}
                          ...
                          const char *expected_routing_id_)
{
    ...
    void *handler = zmq_socket (get_test_context (), SZMQ_REP);
    ...
    zmq_pollitem_t items[] = {
      {control, 0, SZMQ_POLLIN, 0},
      {handler, 0, SZMQ_POLLIN, 0},
    };
    ...
    while (zmq_poll (items, numitems, -1) >= 0) {
        ...
        if (authentication_succeeded) {
            ...
            send_string_expect_success (handler, status_code, SZMQ_SNDMORE);
            send_string_expect_success (handler, "OK", SZMQ_SNDMORE);
            send_string_expect_success (handler, "anonymous", SZMQ_SNDMORE);
            if (zap_protocol_ == zap_too_many_parts) {
                send_string_expect_success (handler, "", SZMQ_SNDMORE);
            }
            if (zap_protocol_ != zap_do_not_send)
                send_string_expect_success (handler, "", 0);
        } else {
            send_string_expect_success (handler, "400", SZMQ_SNDMORE);
                                            ...
                                            const char *monitor_endpoint_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_monitor (
      ...
      SZMQ_EVENT_HANDSHAKE_SUCCEEDED | SZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL
        | SZMQ_EVENT_HANDSHAKE_FAILED_AUTH
        | SZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL));

    //  Create socket for collecting monitor events
    *server_mon_ = test_context_socket (SZMQ_PAIR);
    int linger = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (*server_mon_, SZMQ_LINGER, &linger, sizeof (linger)));
                                    ...
                                    const char *routing_id_)
{
    ...
    *zap_control_ = test_context_socket (SZMQ_REP);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_bind (*zap_control_, "inproc://handler-control"));
    int linger = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (*zap_control_, SZMQ_LINGER, &linger, sizeof (linger)));
    ...
    *server_ = test_context_socket (SZMQ_DEALER);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (*server_, SZMQ_LINGER, &linger, sizeof (linger)));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      *server_, SZMQ_MAXMSGSIZE, &max_msg_size, sizeof (int64_t)));

    socket_config_ (*server_, socket_config_data_);

    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      *server_, SZMQ_ROUTING_ID, routing_id_, strlen (routing_id_)));
                                 ...
                                 void **client_mon_)
{
    ...
    int64_t max_msg_size = 64 * 1024 * 1024;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_MAXMSGSIZE, &max_msg_size, sizeof (int64_t)));
