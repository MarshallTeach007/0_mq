...
void test_sockopt_router_notify ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_getsockopt (
      router, SZMQ_ROUTER_NOTIFY, &opt_notify_read, &opt_notify_read_size));

    TEST_ASSERT_EQUAL (0, opt_notify_read);


    // valid value - Connect
    opt_notify = SZMQ_NOTIFY_CONNECT;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      router, SZMQ_ROUTER_NOTIFY, &opt_notify, sizeof (opt_notify)));
    ...
    opt_notify = SZMQ_NOTIFY_DISCONNECT;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      router, SZMQ_ROUTER_NOTIFY, &opt_notify, sizeof (opt_notify)));
    ...
    // valid value - Both
    opt_notify = SZMQ_NOTIFY_CONNECT | SZMQ_NOTIFY_DISCONNECT;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      router, SZMQ_ROUTER_NOTIFY, &opt_notify, sizeof (opt_notify)));
    ...
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL, zmq_setsockopt (router, SZMQ_ROUTER_NOTIFY, &opt_notify,
                              sizeof (opt_notify)));

    opt_notify = (SZMQ_NOTIFY_CONNECT | SZMQ_NOTIFY_DISCONNECT) + 1;
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL, zmq_setsockopt (router, SZMQ_ROUTER_NOTIFY, &opt_notify,
                              sizeof (opt_notify)));
    ...
    TEST_ASSERT_EQUAL (SZMQ_NOTIFY_CONNECT | SZMQ_NOTIFY_DISCONNECT,
                       opt_notify_read);
    ...
    void *dealer = test_context_socket (SZMQ_DEALER);

    // setsockopt fails for non-router sockets
    opt_notify = SZMQ_NOTIFY_CONNECT;
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL, zmq_setsockopt (dealer, SZMQ_ROUTER_NOTIFY, &opt_notify,
                              sizeof (opt_notify)));

    // getsockopts returns off for any non-router socket
    TEST_ASSERT_SUCCESS_ERRNO (zmq_getsockopt (
      dealer, SZMQ_ROUTER_NOTIFY, &opt_notify_read, &opt_notify_read_size));
...
void test_router_notify_helper (int opt_notify_)
{
    ...
    // valid values
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      router, SZMQ_ROUTER_NOTIFY, &opt_notify_, sizeof (opt_notify_)));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer, SZMQ_ROUTING_ID, dealer_routing_id, 1));

    // dealer connects
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (dealer, connect_address));

    // connection notification msg
    if (opt_notify_ & SZMQ_NOTIFY_CONNECT) {
        ...
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_getsockopt (router, SZMQ_RCVMORE, &opt_more, &opt_more_length));
    ...
    msleep (SETTLE_TIME);
    zmq_getsockopt (dealer, SZMQ_EVENTS, &opt_events, &opt_events_length);

    // connection notification msg
    if (opt_notify_ & SZMQ_NOTIFY_DISCONNECT) {
        // routing-id only message of the connect
        recv_string_expect_success (router, dealer_routing_id,
                                    0);             // 1st part: routing-id
        recv_string_expect_success (router, "", 0); // 2nd part: empty
...
{
    test_router_notify_helper (SZMQ_NOTIFY_CONNECT);
}


void test_router_notify_disconnect ()
{
    test_router_notify_helper (SZMQ_NOTIFY_DISCONNECT);
}


void test_router_notify_both ()
{
    test_router_notify_helper (SZMQ_NOTIFY_CONNECT | SZMQ_NOTIFY_DISCONNECT);
}
...
void test_handshake_fail ()
{
    ...
    void *router = test_context_socket (SZMQ_ROUTER);
    int opt_timeout = 200;
    int opt_notify = SZMQ_NOTIFY_CONNECT | SZMQ_NOTIFY_DISCONNECT;
    char connect_address[MAX_SOCKET_STRING];
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      router, SZMQ_RCVTIMEO, &opt_timeout, sizeof (opt_timeout)));

    bind_loopback_ipv4 (router, connect_address, sizeof connect_address);

    // send something on raw tcp
    void *stream = test_context_socket (SZMQ_STREAM);
...
void test_error_during_multipart ()
{
    ...
    int opt_notify = SZMQ_NOTIFY_DISCONNECT;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      router, SZMQ_ROUTER_NOTIFY, &opt_notify, sizeof (opt_notify)));

    int64_t opt_maxmsgsize = 64; // the handshake fails if this is too small
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      router, SZMQ_MAXMSGSIZE, &opt_maxmsgsize, sizeof (opt_maxmsgsize)));
    ...
    send_string_expect_success (dealer, "Hello2", SZMQ_SNDMORE);
