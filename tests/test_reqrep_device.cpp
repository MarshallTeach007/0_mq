...
void test_roundtrip ()
{
    ...
    //  Create a worker.
    void *rep = test_context_socket (SZMQ_REP);
    ...
    //  Send a request.
    send_string_expect_success (req, "ABC", SZMQ_SNDMORE);
    ...
    for (int i = 0; i != 4; i++) {
        ...
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_getsockopt (router, SZMQ_RCVMORE, &rcvmore, &sz));
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_msg_send (&msg, dealer, rcvmore ? SZMQ_SNDMORE : 0));
    }
    ...
    size_t sz = sizeof (rcvmore);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (rep, SZMQ_RCVMORE, &rcvmore, &sz));
    ...
    //  Send the reply.
    send_string_expect_success (rep, "GHI", SZMQ_SNDMORE);
    ...
    for (int i = 0; i != 4; i++) {
        ...
        size_t sz = sizeof (rcvmore);
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_getsockopt (dealer, SZMQ_RCVMORE, &rcvmore, &sz));
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_msg_send (&msg, router, rcvmore ? SZMQ_SNDMORE : 0));
    }

    //  Receive the reply.
    recv_string_expect_success (req, "GHI", 0);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (req, SZMQ_RCVMORE, &rcvmore, &sz));
    TEST_ASSERT_TRUE (rcvmore);
