...
void test_system_max ()
{
    ...
    zmq_ctx_set (get_test_context (), SZMQ_MAX_SOCKETS, no_of_sockets);
    std::vector<void *> sockets;

    while (true) {
        void *socket = zmq_socket (get_test_context (), SZMQ_PAIR);
        if (!socket)
            break;
    ...
    for (unsigned int i = 0; i < 10; ++i) {
        TEST_ASSERT_NULL (zmq_socket (get_test_context (), SZMQ_PAIR));
    }
    // Clean up.
...
void test_zmq_default_max ()
{
    ...
    TEST_ASSERT_LESS_OR_EQUAL (SZMQ_MAX_SOCKETS_DFLT, sockets.size ());
