...
static void client_task (void *db_)
{
    ...
    void *endpoint = zmq_socket (get_test_context (), SZMQ_PAIR);
    TEST_ASSERT_NOT_NULL (endpoint);
    int linger = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (endpoint, SZMQ_LINGER, &linger, sizeof (linger)));
    ...
    void *client = zmq_socket (get_test_context (), SZMQ_DEALER);
    TEST_ASSERT_NOT_NULL (client);

    // Control socket receives terminate command from main over inproc
    void *control = zmq_socket (get_test_context (), SZMQ_SUB);
    TEST_ASSERT_NOT_NULL (control);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (control, SZMQ_SUBSCRIBE, "", 0));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (control, SZMQ_LINGER, &linger, sizeof (linger)));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      client, SZMQ_ROUTING_ID, routing_id,
      ROUTING_ID_SIZE)); // includes '\0' as an helper for printf
    linger = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_LINGER, &linger, sizeof (linger)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (client, my_endpoint));

    zmq_pollitem_t items[] = {{client, 0, SZMQ_POLLIN, 0},
    ...
    while (run) {
        ...
        for (centitick = 0; centitick < 20; centitick++) {
            ...
            if (items[0].revents & SZMQ_POLLIN) {
                ...
                TEST_ASSERT_SUCCESS_ERRNO (
                  zmq_getsockopt (client, SZMQ_RCVMORE, &rcvmore, &sz));
...
void server_task (void * /*unused_*/)
{
    ...
    void *frontend = zmq_socket (get_test_context (), SZMQ_ROUTER);
    TEST_ASSERT_NOT_NULL (frontend);
    int linger = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (frontend, SZMQ_LINGER, &linger, sizeof (linger)));
    bind_loopback_ipv4 (frontend, my_endpoint, sizeof my_endpoint);

    // Backend socket talks to workers over inproc
    void *backend = zmq_socket (get_test_context (), SZMQ_DEALER);
    TEST_ASSERT_NOT_NULL (backend);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (backend, SZMQ_LINGER, &linger, sizeof (linger)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (backend, "inproc://backend"));

    // Control socket receives terminate command from main over inproc
    void *control = zmq_socket (get_test_context (), SZMQ_REP);
    TEST_ASSERT_NOT_NULL (control);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (control, SZMQ_LINGER, &linger, sizeof (linger)));
    ...
    for (int i = 0; i < QT_CLIENTS; ++i) {
        endpoint_receivers[i] = zmq_socket (get_test_context (), SZMQ_PAIR);
        TEST_ASSERT_NOT_NULL (endpoint_receivers[i]);
        TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
          endpoint_receivers[i], SZMQ_LINGER, &linger, sizeof (linger)));
...
static void server_worker (void * /*unused_*/)
{
    void *worker = zmq_socket (get_test_context (), SZMQ_DEALER);
    TEST_ASSERT_NOT_NULL (worker);
    int linger = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (worker, SZMQ_LINGER, &linger, sizeof (linger)));
    ...
    while (run) {
        int rc = zmq_recv (control, content, CONTENT_SIZE_MAX,
                           SZMQ_DONTWAIT); // usually, rc == -1 (no message)
        if (rc > 0) {
            content[rc] = 0; // NULL-terminate the command string
            if (is_verbose)
        ...
        // if we don't poll, we have to use SZMQ_DONTWAIT, if we poll, we can block-receive with 0
        rc = zmq_recv (worker, routing_id, ROUTING_ID_SIZE_MAX, SZMQ_DONTWAIT);
...
uint64_t recv_stat (void *sock_, bool last_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (sock_, SZMQ_RCVMORE, &more, &moresz));
...
void test_proxy ()
{
    ...
    // Control socket receives terminate command from main over inproc
    void *control = test_context_socket (SZMQ_PUB);
    int linger = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (control, SZMQ_LINGER, &linger, sizeof (linger)));
    ...
    // Control socket receives terminate command from main over inproc
    void *control_proxy = test_context_socket (SZMQ_REQ);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (control_proxy, SZMQ_LINGER, &linger, sizeof (linger)));
    TEST_ASSERT_SUCCESS_ERRNO (
