...
static void zap_handler (void *zap_)
{
    ...
    while (true) {
        ...
        send_string_expect_success (zap_, version, SZMQ_SNDMORE);
        send_string_expect_success (zap_, sequence, SZMQ_SNDMORE);
        if (streq (username, "admin") && streq (password, "password")) {
            send_string_expect_success (zap_, "200", SZMQ_SNDMORE);
            send_string_expect_success (zap_, "OK", SZMQ_SNDMORE);
            send_string_expect_success (zap_, "anonymous", SZMQ_SNDMORE);
            send_string_expect_success (zap_, "", 0);
        } else {
            send_string_expect_success (zap_, "400", SZMQ_SNDMORE);
            send_string_expect_success (zap_, "Invalid username or password",
                                        SZMQ_SNDMORE);
            send_string_expect_success (zap_, "", SZMQ_SNDMORE);
            send_string_expect_success (zap_, "", 0);
        }
        free (version);
        free (sequence);
...
static void setup_server ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, SZMQ_ROUTING_ID, "IDENT", 6));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, SZMQ_ZAP_DOMAIN, domain, strlen (domain)));
    const int as_server = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, SZMQ_PLAIN_SERVER, &as_server, sizeof (int)));
...
void test_plain_success ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_PLAIN_USERNAME, username, strlen (username)));
    const char password[] = "password";
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_PLAIN_PASSWORD, password, strlen (password)));
...
void test_plain_client_as_server_fails ()
{
    ...
    void *client = test_context_socket (SZMQ_DEALER);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_ZAP_DOMAIN, domain, strlen (domain)));
    const int as_server = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_PLAIN_SERVER, &as_server, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (client, my_endpoint));
