...
void test_single_connect (int ipv6_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sc, SZMQ_IPV6, &ipv6_, sizeof (int)));
...
void test_multi_connect (int ipv6_)
{
    ...
    void *sb0 = test_context_socket (SZMQ_REP);
    bind_loopback (sb0, ipv6_, my_endpoint_0, len);

    void *sb1 = test_context_socket (SZMQ_REP);
    bind_loopback (sb1, ipv6_, my_endpoint_1, len);

    void *sb2 = test_context_socket (SZMQ_REP);
...
void test_multi_connect_same_port (int ipv6_)
{
    ...
    void *sc0 = test_context_socket (SZMQ_REQ);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sc0, SZMQ_IPV6, &ipv6_, sizeof (int)));
    ...
    void *sc1 = test_context_socket (SZMQ_REQ);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sc1, SZMQ_IPV6, &ipv6_, sizeof (int)));
