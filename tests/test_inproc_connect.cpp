...
static void pusher (void * /*unused*/)
{
    ...
    void *connect_socket = zmq_socket (get_test_context (), SZMQ_PAIR);
...
static void simult_bind (void *endpt_)
{
    ...
    // Bind
    // do not use test_context_socket here, as it is not thread-safe
    void *bind_socket = zmq_socket (get_test_context (), SZMQ_PAIR);
...
{
    // Bind first
    void *bind_socket = test_context_socket (SZMQ_PAIR);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (bind_socket, "inproc://bbc"));

    // Now connect
    void *connect_socket = test_context_socket (SZMQ_PAIR);
...
{
    // Connect first
    void *connect_socket = test_context_socket (SZMQ_PUB);
    ...
    // Now bind
    void *bind_socket = test_context_socket (SZMQ_SUB);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (bind_socket, SZMQ_SUBSCRIBE, "", 0));
...
void test_connect_before_bind_ctx_term ()
{
    for (int i = 0; i < 20; ++i) {
        // Connect first
        void *connect_socket = test_context_socket (SZMQ_ROUTER);
...
void test_multiple_connects ()
{
    ...
    for (unsigned int i = 0; i < no_of_connects; ++i) {
        connect_socket[i] = test_context_socket (SZMQ_PUSH);
        TEST_ASSERT_SUCCESS_ERRNO (
...
void test_routing_id ()
{
    ...
    TEST_ASSERT_EQUAL_INT (
      1, TEST_ASSERT_SUCCESS_ERRNO (zmq_send (sc, "A", 1, SZMQ_SNDMORE)));
    TEST_ASSERT_EQUAL_INT (
...
void test_unbind ()
{
    ...
    void *bind_socket1 = test_context_socket (SZMQ_PAIR);
    ...
    // Bind socket 2
    void *bind_socket2 = test_context_socket (SZMQ_PAIR);
