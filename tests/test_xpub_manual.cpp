...
void test_basic ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pub, SZMQ_XPUB_MANUAL, &manual, 4));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (pub, "inproc://soname"));

    //  Create a subscriber
    void *sub = test_context_socket (SZMQ_XSUB);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_SUBSCRIBE, "B", 1));

    // Sending A message and B Message
    send_string_expect_success (pub, "A", 0);
    send_string_expect_success (pub, "B", 0);

    recv_string_expect_success (sub, "B", SZMQ_DONTWAIT);
...
void test_unsubscribe_manual ()
{
    ...
    int manual = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pub, SZMQ_XPUB_MANUAL, &manual, sizeof (manual)));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_SUBSCRIBE, "XA", 2));

    // Receive subscription "B" from subscriber
    recv_array_expect_success (pub, subscription2, 0);

    // Subscribe socket for XB instead
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_SUBSCRIBE, "XB", 2));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_UNSUBSCRIBE, "XA", 2));
    ...
    recv_string_expect_success (sub, "XB", SZMQ_DONTWAIT);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_UNSUBSCRIBE, "XB", 2));
...
void test_xpub_proxy_unsubscribe_on_disconnect ()
{
    ...
    recv_array_expect_success (xpub_proxy, subscription, SZMQ_DONTWAIT);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (xpub_proxy, SZMQ_SUBSCRIBE, topic_buff, 1));
    send_array_expect_success (xsub_proxy, subscription, 0);

    // second subscriber subscribes
    void *sub2 = test_context_socket (SZMQ_SUB);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (sub2, my_endpoint_backend));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub2, SZMQ_SUBSCRIBE, topic_buff, 1));
    ...
    send_array_expect_success (pub, topic_buff, SZMQ_SNDMORE);
    ...
    recv_array_expect_success (xsub_proxy, topic_buff, SZMQ_DONTWAIT);
    recv_array_expect_success (xsub_proxy, payload_buff, SZMQ_DONTWAIT);
    send_array_expect_success (xpub_proxy, topic_buff, SZMQ_SNDMORE);
    ...
    recv_array_expect_success (sub2, topic_buff, SZMQ_DONTWAIT);
    recv_array_expect_success (sub2, payload_buff, SZMQ_DONTWAIT);

    recv_array_expect_success (sub1, topic_buff, SZMQ_DONTWAIT);
    recv_array_expect_success (sub1, payload_buff, SZMQ_DONTWAIT);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (xpub_proxy, SZMQ_UNSUBSCRIBE, topic_buff, 1));
    ...
    TEST_ASSERT_FAILURE_ERRNO (
      EAGAIN, zmq_recv (xsub_proxy, buffer, sizeof buffer, SZMQ_DONTWAIT));
...
void test_missing_subscriptions ()
{
    ...
    void *sub1 = test_context_socket (SZMQ_SUB);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (sub1, my_endpoint_backend));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub1, SZMQ_SUBSCRIBE, topic1, 1));
    ...
    recv_array_expect_success (xpub_proxy, subscription1, SZMQ_DONTWAIT);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (xpub_proxy, SZMQ_SUBSCRIBE, topic1, 1));
    send_array_expect_success (xsub_proxy, subscription1, 0);

    // second subscriber
    void *sub2 = test_context_socket (SZMQ_SUB);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (sub2, my_endpoint_backend));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub2, SZMQ_SUBSCRIBE, topic2, 1));

    // wait
    msleep (SETTLE_TIME);

    const uint8_t subscription2[] = {1, static_cast<uint8_t> (topic2[0])};
    recv_array_expect_success (xpub_proxy, subscription2, SZMQ_DONTWAIT);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (xpub_proxy, SZMQ_SUBSCRIBE, topic2, 1));
    ...
    send_string_expect_success (pub, topic1, SZMQ_SNDMORE);
    send_string_expect_success (pub, payload, 0);
    send_string_expect_success (pub, topic2, SZMQ_SNDMORE);
    send_string_expect_success (pub, payload, 0);
    ...
    // proxy reroutes data messages to subscribers
    recv_string_expect_success (xsub_proxy, topic1, SZMQ_DONTWAIT);
    recv_string_expect_success (xsub_proxy, payload, SZMQ_DONTWAIT);
    send_string_expect_success (xpub_proxy, topic1, SZMQ_SNDMORE);
    send_string_expect_success (xpub_proxy, payload, 0);

    recv_string_expect_success (xsub_proxy, topic2, SZMQ_DONTWAIT);
    recv_string_expect_success (xsub_proxy, payload, SZMQ_DONTWAIT);
    send_string_expect_success (xpub_proxy, topic2, SZMQ_SNDMORE);
    send_string_expect_success (xpub_proxy, payload, 0);
    ...
    // each subscriber should now get a message
    recv_string_expect_success (sub2, topic2, SZMQ_DONTWAIT);
    recv_string_expect_success (sub2, payload, SZMQ_DONTWAIT);

    recv_string_expect_success (sub1, topic1, SZMQ_DONTWAIT);
    recv_string_expect_success (sub1, payload, SZMQ_DONTWAIT);
...
void test_unsubscribe_cleanup ()
{
    ...
    char buffer[2];
    TEST_ASSERT_FAILURE_ERRNO (
      EAGAIN, zmq_recv (sub, buffer, sizeof buffer, SZMQ_DONTWAIT));
    ...
    // reconnect
    sub = test_context_socket (SZMQ_XSUB);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (sub, my_endpoint));
...
#ifdef SZMQ_ONLY_FIRST_SUBSCRIBE
void test_user_message_multi ()
{
    ...
    void *pub = test_context_socket (SZMQ_XPUB);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (pub, "inproc://soname"));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (pub, SZMQ_ONLY_FIRST_SUBSCRIBE,
    ...
    void *sub = test_context_socket (SZMQ_XSUB);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (sub, "inproc://soname"));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub, SZMQ_ONLY_FIRST_SUBSCRIBE,
                                               &only_first_subscribe,
                                               sizeof (only_first_subscribe)));
    ...
    send_array_expect_success (sub, msg_common, SZMQ_SNDMORE);
    ...
    send_array_expect_success (sub, msg_1a, SZMQ_SNDMORE);
    send_array_expect_success (sub, msg_1b, 0);
    recv_array_expect_success (pub, msg_1a, 0);
    recv_array_expect_success (pub, msg_1b, 0);

    send_array_expect_success (sub, msg_0a, SZMQ_SNDMORE);
