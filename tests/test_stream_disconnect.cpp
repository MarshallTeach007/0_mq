...
bool has_more (void *socket_)
{
    ...
    int rc = zmq_getsockopt (socket_, SZMQ_RCVMORE, &more, &more_size);
    if (rc != 0)
        return false;
    return more != 0;
}
...
void test_stream_disconnect ()
{
    ...
    sockets[SERVER] = test_context_socket (SZMQ_STREAM);
    int enabled = 1;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      sockets[SERVER], SZMQ_STREAM_NOTIFY, &enabled, sizeof (enabled)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (sockets[SERVER], "tcp://0.0.0.0:*"));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (sockets[SERVER], SZMQ_LAST_ENDPOINT, bind_endpoint, &len));
    ...
    sockets[CLIENT] = test_context_socket (SZMQ_STREAM);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
      sockets[CLIENT], SZMQ_STREAM_NOTIFY, &enabled, sizeof (enabled)));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (sockets[CLIENT], SZMQ_ROUTING_ID, blob_data, &blob_size));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_msg_send (&msg, sockets[dialog[0].turn], SZMQ_SNDMORE));
    ...
    while (step < steps) {
        ...
        zmq_pollitem_t items[] = {
          {sockets[SERVER], 0, SZMQ_POLLIN, 0},
          {sockets[CLIENT], 0, SZMQ_POLLIN, 0},
        };
        TEST_ASSERT_SUCCESS_ERRNO (zmq_poll (items, 2, 100));

        // Check for data received by the server.
        if (items[SERVER].revents & SZMQ_POLLIN) {
            ...
            if (size == 0) {
                ...
                // Send the response.
                TEST_ASSERT_SUCCESS_ERRNO (
                  zmq_msg_send (&peer_frame, sockets[SERVER], SZMQ_SNDMORE));
                TEST_ASSERT_SUCCESS_ERRNO (
                  zmq_msg_send (&data_frame, sockets[SERVER], SZMQ_SNDMORE));
            }
        ...
        if (items[CLIENT].revents & SZMQ_POLLIN) {
            ...
            // Send the response.
            TEST_ASSERT_SUCCESS_ERRNO (
              zmq_msg_send (&peer_frame, sockets[CLIENT], SZMQ_SNDMORE));
            TEST_ASSERT_SUCCESS_ERRNO (
              zmq_msg_send (&data_frame, sockets[CLIENT], SZMQ_SNDMORE));
