...
void test_xpub_verbose_one_sub ()
{
    ...
    //  Subscribe for A
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub, SZMQ_SUBSCRIBE, topic_a, 1));

    // Receive subscriptions from subscriber
    recv_array_expect_success (pub, subscribe_a_msg, 0);

    // Subscribe socket for B instead
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (sub, SZMQ_SUBSCRIBE, topic_b, 1));
    ...
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN, zmq_recv (pub, NULL, 0, SZMQ_DONTWAIT));

    int verbose = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pub, SZMQ_XPUB_VERBOSE, &verbose, sizeof (int)));
...
void create_duplicate_subscription (void *pub_, void *sub0_, void *sub1_)
{
    //  Subscribe for A
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub0_, SZMQ_SUBSCRIBE, topic_a, 1));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub1_, SZMQ_SUBSCRIBE, topic_a, 1));

    //  This time it is duplicated, so it will be filtered out by XPUB
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN, zmq_recv (pub_, NULL, 0, SZMQ_DONTWAIT));
}
...
void test_xpub_verbose_two_subs ()
{
    ...
    // Subscribe socket for B instead
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub0, SZMQ_SUBSCRIBE, topic_b, 1));
    ...
    // Subscribe socket for A again
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub1, SZMQ_SUBSCRIBE, topic_a, 1));
...
void test_xpub_verboser_one_sub ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub, SZMQ_UNSUBSCRIBE, topic_a, 1));
    ...
    int verbose = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pub, SZMQ_XPUB_VERBOSER, &verbose, sizeof (int)));
...
void test_xpub_verboser_two_subs ()
{
    ...
    //  Unsubscribe for A, this time it exists in XPUB
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub0, SZMQ_UNSUBSCRIBE, topic_a, 1));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub1, SZMQ_UNSUBSCRIBE, topic_a, 1));
    ...
    // Subscribe socket for A again
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sub0, SZMQ_SUBSCRIBE, topic_a, 1));
