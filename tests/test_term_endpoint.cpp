...
#if !defined SZMQ_HAVE_WINDOWS && !defined SZMQ_HAVE_OPENVMS
...
void test_send_after_unbind_fails ()
{
    ...
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN, zmq_send (push, "ABC", 3, SZMQ_DONTWAIT));
...
void test_unbind_via_last_endpoint ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (pull, SZMQ_LAST_ENDPOINT, my_endpoint, &buf_size));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_unbind (pull, my_endpoint));
#endif
#if defined SZMQ_HAVE_VMCI
    buf_size = sizeof (my_endpoint);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (req, SZMQ_LAST_ENDPOINT, my_endpoint, &buf_size));
