...
static void bounce_fail (void *server_, void *client_)
{
    ...
    send_string_expect_success (client_, content, SZMQ_SNDMORE);
    send_string_expect_success (client_, content, 0);

    //  Receive message at server side (should not succeed)
    int timeout = SETTLE_TIME;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server_, SZMQ_RCVTIMEO, &timeout, sizeof (int)));
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN, zmq_recv (server_, buffer, 32, 0));

    //  Send message from server to client to test other direction
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server_, SZMQ_SNDTIMEO, &timeout, sizeof (int)));
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN,
                               zmq_send (server_, content, 32, SZMQ_SNDMORE));
}
...
{
    void *sb = test_context_socket (SZMQ_DEALER);
    ...
    void *sc = test_context_socket (SZMQ_DEALER);

    // If a test fails, don't hang for too long
    int timeout = 2500;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sb, SZMQ_RCVTIMEO, &timeout, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sb, SZMQ_SNDTIMEO, &timeout, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sc, SZMQ_RCVTIMEO, &timeout, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sc, SZMQ_SNDTIMEO, &timeout, sizeof (int)));
    int interval = -1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (sc, SZMQ_RECONNECT_IVL, &interval, sizeof (int)));
...
{
    run_test<uid_t> (SZMQ_IPC_FILTER_UID, getuid (), 0, 1);
}
void test_filter_with_possibly_nonexistent_uid ()
{
    run_test<uid_t> (SZMQ_IPC_FILTER_UID, getuid () + 1, 0, -1);
}
void test_filter_with_process_owner_gid ()
{
    run_test<gid_t> (SZMQ_IPC_FILTER_GID, group, 0, 1);
}
void test_filter_with_supplemental_process_owner_gid ()
{
    run_test<gid_t> (SZMQ_IPC_FILTER_GID, supgroup, 0, 1);
}
void test_filter_with_possibly_nonexistent_gid ()
{
    run_test<gid_t> (SZMQ_IPC_FILTER_GID, notgroup, 0, -1);
}
#if defined SZMQ_HAVE_SO_PEERCRED
void test_filter_with_current_process_pid ()
{
    run_test<pid_t> (SZMQ_IPC_FILTER_PID, getpid (), 0, 1);
}
void test_filter_with_possibly_nonexistent_pid ()
{
    run_test<pid_t> (SZMQ_IPC_FILTER_PID, getpid () + 1, 0, -1);
}
#else
void test_filter_with_pid_fails ()
{
    ...
    run_test<pid_t> (SZMQ_IPC_FILTER_PID, getpid (), EINVAL, 0);
}
#endif
#else
void test_filter_with_zero_uid_fails ()
{
    run_test<uid_t> (SZMQ_IPC_FILTER_UID, 0, EINVAL, 0);
}
void test_filter_with_zero_gid_fails ()
{
    run_test<gid_t> (SZMQ_IPC_FILTER_GID, 0, EINVAL, 0);
}
void test_filter_with_zero_pid_fails ()
{
    run_test<pid_t> (SZMQ_IPC_FILTER_PID, 0, EINVAL, 0);
}
#endif // defined SZMQ_HAVE_SO_PEERCRED || defined SZMQ_HAVE_LOCAL_PEERCRED
