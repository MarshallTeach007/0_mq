...
bool send_msg_to_peer_if_ready (void *router_, const char *peer_routing_id_)
{
    ...
    if (rc & SZMQ_POLLOUT) {
        send_string_expect_success (router_, peer_routing_id_,
                                    SZMQ_SNDMORE | SZMQ_DONTWAIT);
        send_string_expect_success (router_, "Hello", SZMQ_DONTWAIT);
...
void test_get_peer_state ()
{
    ...
    int mandatory = 1;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (router, SZMQ_ROUTER_MANDATORY,
                                               &mandatory, sizeof (mandatory)));

    const char *my_endpoint = "inproc://test_get_peer_state";
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (router, my_endpoint));

    void *dealer1 = test_context_socket (SZMQ_DEALER);
    void *dealer2 = test_context_socket (SZMQ_DEALER);

    //  Lower HWMs to allow doing the test with fewer messages
    const int hwm = 100;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (router, SZMQ_SNDHWM, &hwm, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer1, SZMQ_RCVHWM, &hwm, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer2, SZMQ_RCVHWM, &hwm, sizeof (int)));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer1, SZMQ_ROUTING_ID, dealer1_routing_id, 1));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (dealer1, my_endpoint));

    //  Name dealer2 "Y" and connect it to our router
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer2, SZMQ_ROUTING_ID, dealer2_routing_id, 1));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_poller_add (poller, router, NULL, SZMQ_POLLOUT));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_poller_add (poller, dealer1, NULL, SZMQ_POLLIN));
    ...
    for (unsigned int iteration = 0; iteration < count; ++iteration) {
        ...
        for (unsigned int event_no = 0; event_no < event_size; ++event_no) {
            ...
            if (current_event.socket == router
                && current_event.events & SZMQ_POLLOUT) {
            ...
            if (current_event.socket == dealer1
                && current_event.events & SZMQ_POLLIN) {
                recv_string_expect_success (dealer1, "Hello", SZMQ_DONTWAIT);
                int more;
                size_t more_size = sizeof (more);
                TEST_ASSERT_SUCCESS_ERRNO (
                  zmq_getsockopt (dealer1, SZMQ_RCVMORE, &more, &more_size));
...
void test_basic ()
{
    ...
    send_string_expect_success (router, "UNKNOWN", SZMQ_SNDMORE);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (router, SZMQ_ROUTER_MANDATORY,
                                               &mandatory, sizeof (mandatory)));
    int rc = zmq_send (router, "UNKNOWN", 7, SZMQ_SNDMORE);
    TEST_ASSERT_EQUAL_INT (-1, rc);
    TEST_ASSERT_EQUAL_INT (EHOSTUNREACH, errno);

    //  Create dealer called "X" and connect it to our router
    void *dealer = test_context_socket (SZMQ_DEALER);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (dealer, SZMQ_ROUTING_ID, "X", 1));
    ...
    //  It should work
    send_string_expect_success (router, "X", SZMQ_SNDMORE);
