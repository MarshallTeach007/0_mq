...
extern "C" int LLVMFuzzerTestOneInput (const uint8_t *data, size_t size)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, SZMQ_MAXMSGSIZE, &max_msg_size, sizeof (int64_t)));
    ...
    void *client_good = test_context_socket (SZMQ_DEALER);
    ...
    zmq_msg_t msg;
    zmq_msg_init (&msg);
    while (-1 != zmq_msg_recv (&msg, server, SZMQ_DONTWAIT)) {
        zmq_msg_close (&msg);
        zmq_msg_init (&msg);
    }
...
#ifndef SZMQ_USE_FUZZING_ENGINE
