...
void test_router_mandatory_hwm ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (router, SZMQ_SNDHWM, &sndhwm, sizeof (sndhwm)));
    int linger = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (router, SZMQ_LINGER, &linger, sizeof (linger)));
    ...
    int rcvhwm = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer, SZMQ_RCVHWM, &rcvhwm, sizeof (rcvhwm)));
    ...
    for (i = 0; i < 100000; ++i) {
        ...
        const int rc = zmq_send (router, "X", 1, SZMQ_DONTWAIT | SZMQ_SNDMORE);
        if (rc == -1 && zmq_errno () == EAGAIN)
            break;
        TEST_ASSERT_EQUAL_INT (1, rc);
        send_array_expect_success (router, buf, SZMQ_DONTWAIT);
    }
