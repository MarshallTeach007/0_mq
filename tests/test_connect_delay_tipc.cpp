...
void test_send_one_connected_one_unconnected ()
{
    ...
    void *to = test_context_socket (SZMQ_PULL);
    int timeout = 5000;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (to, SZMQ_LINGER, &timeout, sizeof (timeout)));

    // Bind the one valid receiver
    val = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (to, SZMQ_LINGER, &val, sizeof (val)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (to, "tipc://{6555,0,0}"));

    // Create a socket pushing to two endpoints - only 1 message should arrive.
    void *from = test_context_socket (SZMQ_PUSH);

    val = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (from, SZMQ_LINGER, &val, sizeof (val)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (from, SZMQ_LINGER, &timeout, sizeof (timeout)));
    ...
    timeout = SETTLE_TIME;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (to, SZMQ_RCVTIMEO, &timeout, sizeof (int)));
...
void test_send_one_connected_one_unconnected_with_delay ()
{
    ...
    val = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (from, SZMQ_DELAY_ATTACH_ON_CONNECT, &val, sizeof (val)));
...
void test_send_disconnected_with_delay ()
{
    ...
    void *backend = test_context_socket (SZMQ_DEALER);
    void *frontend = test_context_socket (SZMQ_DEALER);
    void *monitor = test_context_socket (SZMQ_PAIR);
    int rc;
    int zero = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (backend, SZMQ_LINGER, &zero, sizeof (zero)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (frontend, SZMQ_LINGER, &zero, sizeof (zero)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_monitor (frontend, "inproc://monitor",
                                                   SZMQ_EVENT_DISCONNECTED));
    int timeout = 5000;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (backend, SZMQ_LINGER, &timeout, sizeof (timeout)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (frontend, SZMQ_LINGER, &timeout, sizeof (timeout)));

    //  Frontend connects to backend using DELAY_ATTACH_ON_CONNECT
    int on = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (frontend, SZMQ_DELAY_ATTACH_ON_CONNECT, &on, sizeof (on)));
    ...
    send_string_expect_success (frontend, "Hello", SZMQ_DONTWAIT);

    test_context_socket_close (backend);

    // Wait for disconnect to happen
    expect_monitor_event (monitor, SZMQ_EVENT_DISCONNECTED);

    // Send a message, might succeed depending on scheduling of the I/O thread
    do {
        rc = zmq_send (frontend, "Hello", 5, SZMQ_DONTWAIT);
        TEST_ASSERT_TRUE (rc == 5 || (rc == -1 && zmq_errno () == EAGAIN));
    } while (rc == 5);

    //  Recreate backend socket
    backend = test_context_socket (SZMQ_DEALER);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (backend, SZMQ_LINGER, &zero, sizeof (zero)));
    TEST_ASSERT_SUCCESS_ERRNO (zmq_bind (backend, "tipc://{5560,0,0}"));
