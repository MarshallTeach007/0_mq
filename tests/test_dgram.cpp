...
{
    send_string_expect_success (s_, address_, SZMQ_SNDMORE);
...
{
    void *socket = test_context_socket (SZMQ_DGRAM);
...
void test_roundtrip ()
{
    ...
    void *sender = test_context_socket (SZMQ_DGRAM);
    void *listener = test_context_socket (SZMQ_DGRAM);
