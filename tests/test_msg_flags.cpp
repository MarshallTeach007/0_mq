...
void test_more ()
{
    ...
    //  Send 2-part message.
    send_string_expect_success (sc, "A", SZMQ_SNDMORE);
...
{
    // Test SZMQ_SHARED property (case 1, refcounted messages)
    zmq_msg_t msg_a;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_msg_init_size (&msg_a, 1024)); // large enough to be a type_lmsg

    // Message is not shared
    TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_get (&msg_a, SZMQ_SHARED));
    ...
    TEST_ASSERT_EQUAL_INT (
      1, TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_get (&msg_b, SZMQ_SHARED)));
...
{
    zmq_msg_t msg_a;
    // Test SZMQ_SHARED property (case 2, constant data messages)
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_msg_init_data (&msg_a, (void *) "TEST", 5, 0, 0));

    // Message reports as shared
    TEST_ASSERT_EQUAL_INT (
      1, TEST_ASSERT_SUCCESS_ERRNO (zmq_msg_get (&msg_a, SZMQ_SHARED)));
