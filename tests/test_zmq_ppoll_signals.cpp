...
void test_ppoll_signals ()
{
    ...
    if (child_pid > 0) { // parent
        setup_test_context ();
        void *socket = test_context_socket (SZMQ_REQ);
        ...
        TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (
          socket, SZMQ_RCVTIMEO, &recv_timeout, sizeof (recv_timeout)));
        ...
        void *socket = test_context_socket (SZMQ_REP);
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_connect (socket, "tcp://127.0.0.1:6660"));

        zmq_pollitem_t pollitems[] = {
          {socket, 0, SZMQ_POLLIN, 0},
        };
        ...
        TEST_ASSERT_BITS_HIGH (SZMQ_POLLIN, pollitems[0].revents);
