...
fd_t get_fd (void *socket_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_getsockopt (socket_, SZMQ_FD, &fd, &fd_size));
    return fd;
}
...
{
    void *socket = test_context_socket (SZMQ_PAIR);
    TEST_ASSERT_FAILURE_ERRNO (EFAULT,
                               zmq_poller_add (NULL, socket, NULL, SZMQ_POLLIN));
...
void test_null_poller_pointers_add_indirect ()
{
    ...
    TEST_ASSERT_FAILURE_ERRNO (
      EFAULT, zmq_poller_add (&null_poller, socket, NULL, SZMQ_POLLIN));
    test_context_socket_close (socket);
}
...
void test_null_poller_pointers_modify_direct ()
{
    void *socket = test_context_socket (SZMQ_PAIR);
    TEST_ASSERT_FAILURE_ERRNO (EFAULT,
                               zmq_poller_modify (NULL, socket, SZMQ_POLLIN));
    test_context_socket_close (socket);
}
...
void test_null_poller_pointers_modify_indirect ()
{
    ...
    void *socket = test_context_socket (SZMQ_PAIR);
    TEST_ASSERT_FAILURE_ERRNO (
      EFAULT, zmq_poller_modify (&null_poller, socket, SZMQ_POLLIN));
    test_context_socket_close (socket);
}
...
void test_null_poller_pointers_add_fd_direct ()
{
    ...
    TEST_ASSERT_FAILURE_ERRNO (EFAULT,
                               zmq_poller_add_fd (NULL, fd, NULL, SZMQ_POLLIN));
    test_context_socket_close (socket);
}
...
void test_null_poller_pointers_add_fd_indirect ()
{
    ...
    void *null_poller = NULL;
    TEST_ASSERT_FAILURE_ERRNO (
      EFAULT, zmq_poller_add_fd (&null_poller, fd, NULL, SZMQ_POLLIN));
    test_context_socket_close (socket);
}
...
void test_null_poller_pointers_modify_fd_direct ()
{
    ...
    const fd_t fd = get_fd (socket);
    TEST_ASSERT_FAILURE_ERRNO (EFAULT,
                               zmq_poller_modify_fd (NULL, fd, SZMQ_POLLIN));
    test_context_socket_close (socket);
}
...
void test_null_poller_pointers_modify_fd_indirect ()
{
    ...
    void *null_poller = NULL;
    TEST_ASSERT_FAILURE_ERRNO (
      EFAULT, zmq_poller_modify_fd (&null_poller, fd, SZMQ_POLLIN));
    test_context_socket_close (socket);
}
...
void test_null_socket_pointers ()
{
    ...
    TEST_ASSERT_FAILURE_ERRNO (ENOTSOCK,
                               zmq_poller_add (poller, NULL, NULL, SZMQ_POLLIN));

    TEST_ASSERT_FAILURE_ERRNO (ENOTSOCK,
                               zmq_poller_modify (poller, NULL, SZMQ_POLLIN));
    ...
    TEST_ASSERT_FAILURE_ERRNO (
      EBADF, zmq_poller_add_fd (poller, null_socket_fd, NULL, SZMQ_POLLIN));

    TEST_ASSERT_FAILURE_ERRNO (
      EBADF, zmq_poller_modify_fd (poller, null_socket_fd, SZMQ_POLLIN));
...
void test_with_valid_poller (extra_poller_func_t extra_func_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_poller_add (poller, socket, NULL, SZMQ_POLLIN));
...
{
    void *socket = test_context_socket (SZMQ_CLIENT);
...
void call_poller_size (void *poller_, void *socket_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_poller_add (poller_, socket_, NULL, SZMQ_POLLIN));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_poller_add_fd (poller_, plain_socket, NULL, SZMQ_POLLOUT));
    rc = zmq_poller_size (poller_);
    TEST_ASSERT_SUCCESS_ERRNO (rc);
...
void call_poller_add_twice_fails (void *poller_, void *socket_)
{
    ...
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL, zmq_poller_add (poller_, socket_, NULL, SZMQ_POLLIN));
...
void call_poller_modify_unregistered_fails (void *poller_, void *socket_)
{
    ...
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL, zmq_poller_modify (poller_, socket_, SZMQ_POLLIN));
}
...
void call_poller_add_fd_twice_fails (void *poller_, void * /*zeromq_socket*/)
{
    fd_t plain_socket = socket (AF_INET, SOCK_STREAM, IPPROTO_TCP);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_poller_add_fd (poller_, plain_socket, NULL, SZMQ_POLLIN));

    //  attempt to add the same plain socket twice
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL, zmq_poller_add_fd (poller_, plain_socket, NULL, SZMQ_POLLIN));
                                               ...
                                               void * /*zeromq_socket*/)
{
    ...
    //  attempt to remove plain socket that is not present
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL, zmq_poller_modify_fd (poller_, plain_socket, SZMQ_POLLIN));
...
{
    //  Create few sockets
    void *vent = test_context_socket (SZMQ_PUSH);

    size_t len = MAX_SOCKET_STRING;
    char my_endpoint[MAX_SOCKET_STRING];
    bind_loopback_ipv4 (vent, my_endpoint, len);

    void *sink = test_context_socket (SZMQ_PULL);
    ...
    // register sink
    TEST_ASSERT_SUCCESS_ERRNO (zmq_poller_add (poller, sink, sink, SZMQ_POLLIN));
...
void test_poll_fd ()
{
    ...
    void *bowl = test_context_socket (SZMQ_PULL);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_poller_add_fd (poller, fd, bowl, SZMQ_POLLIN));
...
{
#if defined(SZMQ_SERVER) && defined(SZMQ_CLIENT)
    //  Create sockets
    void *server = test_context_socket (SZMQ_SERVER);

    size_t len = MAX_SOCKET_STRING;
    char my_endpoint[MAX_SOCKET_STRING];
    bind_loopback_ipv4 (server, my_endpoint, len);

    void *client = test_context_socket (SZMQ_CLIENT);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_poller_add (poller, server, NULL, SZMQ_POLLIN));
    ...
    //  Polling on pollout
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_poller_modify (poller, server, SZMQ_POLLOUT | SZMQ_POLLIN));
    ...
    TEST_ASSERT (event.fd == -1);
#endif
    TEST_ASSERT_EQUAL_INT (SZMQ_POLLOUT, event.events);
