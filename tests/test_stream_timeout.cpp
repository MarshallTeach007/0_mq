...
static void test_stream_handshake_timeout_accept ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (dealer, SZMQ_HANDSHAKE_IVL, &val, &vsize));
    TEST_ASSERT_EQUAL (sizeof (val), vsize);
    TEST_ASSERT_EQUAL_INT (30000, val);
    // make handshake timeout faster - 1/10 sec
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer, SZMQ_HANDSHAKE_IVL, &tenth, sizeof (tenth)));
    vsize = sizeof (val);
    ...
    void *dealer_mon = test_context_socket (SZMQ_PAIR);
    ...
    // we should get SZMQ_EVENT_ACCEPTED and then SZMQ_EVENT_DISCONNECTED
    int event = get_monitor_event (dealer_mon, NULL, NULL);
    TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_ACCEPTED, event);
    event = get_monitor_event (dealer_mon, NULL, NULL);
    TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_DISCONNECTED, event);
...
static void test_stream_handshake_timeout_connect ()
{
    ...
    // we should get SZMQ_EVENT_CONNECTED and then SZMQ_EVENT_DISCONNECTED
    int event = get_monitor_event (dealer_mon, NULL, NULL);
