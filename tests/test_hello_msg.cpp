...
void test (const char *address)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (router, SZMQ_HELLO_MSG, "H", 1));
...
void test_inproc_late_bind ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (server, SZMQ_HELLO_MSG, "W", 1));

    //  Create a dealer
    void *client = test_context_socket (SZMQ_CLIENT);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (client, SZMQ_HELLO_MSG, "H", 1));
