...
void test_change_before_connected ()
{
    ...
    int val = 2;
    rc = zmq_setsockopt (connect_socket, SZMQ_RCVHWM, &val, sizeof (val));
    TEST_ASSERT_EQUAL_INT (0, rc);
    rc = zmq_setsockopt (bind_socket, SZMQ_SNDHWM, &val, sizeof (val));
    TEST_ASSERT_EQUAL_INT (0, rc);
    ...
    val = 0;
    rc = zmq_getsockopt (bind_socket, SZMQ_SNDHWM, &val, &placeholder);
    TEST_ASSERT_EQUAL_INT (0, rc);
...
int send_until_wouldblock (void *socket_)
{
    int send_count = 0;
    while (send_count < MAX_SENDS
           && zmq_send (socket_, &send_count, sizeof (send_count), SZMQ_DONTWAIT)
                == sizeof (send_count)) {
        ++send_count;
    }
    return send_count;
...
void test_decrease_when_full ()
{
    ...
    int sndhwm = 100;
    rc = zmq_setsockopt (bind_socket, SZMQ_SNDHWM, &sndhwm, sizeof (sndhwm));
    TEST_ASSERT_EQUAL_INT (0, rc);
    ...
    rc =
      zmq_getsockopt (bind_socket, SZMQ_SNDHWM, &sndhwm_read, &sndhwm_read_size);
    TEST_ASSERT_EQUAL_INT (0, rc);
    ...
    while (
      ...
      && zmq_recv (connect_socket, &read_data, sizeof (read_data), SZMQ_DONTWAIT)
           == sizeof (read_data)) {
