...
void test_with_handover ()
{
    ...
    int handover = 1;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_setsockopt (router, SZMQ_ROUTER_HANDOVER,
                                               &handover, sizeof (handover)));

    //  Create dealer called "X" and connect it to our router
    void *dealer_one = test_context_socket (SZMQ_DEALER);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer_one, SZMQ_ROUTING_ID, "X", 1));
    ...
    void *dealer_two = test_context_socket (SZMQ_DEALER);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer_two, SZMQ_ROUTING_ID, "X", 1));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer_one, SZMQ_RCVTIMEO, &timeout, sizeof timeout));
...
void test_without_handover ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (router, SZMQ_LAST_ENDPOINT, my_endpoint, &len));
    ...
    const int timeout = SETTLE_TIME;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (router, SZMQ_RCVTIMEO, &timeout, sizeof timeout));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dealer_two, SZMQ_RCVTIMEO, &timeout, sizeof timeout));
