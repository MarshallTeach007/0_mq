...
static int get_monitor_event (void *monitor_)
{
    for (int i = 0; i < 10; i++) {
        ...
        if (zmq_msg_recv (&msg, monitor_, SZMQ_DONTWAIT) == -1) {
            msleep (SETTLE_TIME);
...
static void setup_curve (void *socket_, int is_server_)
{
    ...
    zmq_setsockopt (socket_, SZMQ_CURVE_SECRETKEY, secret_key,
                    strlen (secret_key));
    zmq_setsockopt (socket_, SZMQ_CURVE_PUBLICKEY, public_key,
                    strlen (public_key));
    if (is_server_)
        zmq_setsockopt (socket_, SZMQ_CURVE_SERVER, &is_server_,
                        sizeof (is_server_));
    else
        zmq_setsockopt (socket_, SZMQ_CURVE_SERVERKEY, server_key,
                        strlen (server_key));
}
                                ...
                                int socket_type_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (server, SZMQ_LINGER, &value, sizeof (value)));

    if (set_heartbeats_) {
        value = 50;
        TEST_ASSERT_SUCCESS_ERRNO (
          zmq_setsockopt (server, SZMQ_HEARTBEAT_IVL, &value, sizeof (value)));
    }
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_monitor (
      ...
      SZMQ_EVENT_CONNECTED | SZMQ_EVENT_DISCONNECTED | SZMQ_EVENT_ACCEPTED));
...
static void test_heartbeat_timeout (int server_type_, int mock_ping_)
{
    ...
    TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_ACCEPTED, rc);

    if (!mock_ping_) {
        // We should have been disconnected
        rc = get_monitor_event (server_mon);
        TEST_ASSERT_EQUAL_INT (SZMQ_EVENT_DISCONNECTED, rc);
    }
...
static void test_heartbeat_ttl (int client_type_, int server_type_)
{
    ...
    value = 100;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_HEARTBEAT_TTL, &value, sizeof (value)));

    // Set the heartbeat interval to much longer than the TTL so that
    // the socket times out oon the remote side.
    value = 250;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (client, SZMQ_HEARTBEAT_IVL, &value, sizeof (value)));
...
{
    test_heartbeat_timeout (SZMQ_ROUTER, 0);
}

void test_heartbeat_timeout_router_mock_ping ()
{
    test_heartbeat_timeout (SZMQ_ROUTER, 1);
}
...
DEFINE_TESTS (dealer, router, SZMQ_DEALER, SZMQ_ROUTER)
DEFINE_TESTS (req, rep, SZMQ_REQ, SZMQ_REP)
DEFINE_TESTS (pull, push, SZMQ_PULL, SZMQ_PUSH)
DEFINE_TESTS (sub, pub, SZMQ_SUB, SZMQ_PUB)
DEFINE_TESTS (pair, pair, SZMQ_PAIR, SZMQ_PAIR)

#ifdef SZMQ_BUILD_DRAFT_API
DEFINE_TESTS (gather, scatter, SZMQ_GATHER, SZMQ_SCATTER)
DEFINE_TESTS (client, server, SZMQ_CLIENT, SZMQ_SERVER)
#endif
...
{
    void *const socket = test_context_socket (SZMQ_PAIR);
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (socket, SZMQ_HEARTBEAT_TTL, &value_, sizeof (value_)));

    int value_read;
    size_t value_read_size = sizeof (value_read);
    TEST_ASSERT_SUCCESS_ERRNO (zmq_getsockopt (socket, SZMQ_HEARTBEAT_TTL,
...
void test_setsockopt_heartbeat_ttl_more_than_max_fails ()
{
    ...
    TEST_ASSERT_FAILURE_ERRNO (
      EINVAL,
      zmq_setsockopt (socket, SZMQ_HEARTBEAT_TTL, &value, sizeof (value)));
