...
void test_invalid_rep ()
{
    ...
    void *router_socket = test_context_socket (SZMQ_ROUTER);
    void *req_socket = test_context_socket (SZMQ_REQ);

    int linger = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (router_socket, SZMQ_LINGER, &linger, sizeof (int)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (req_socket, SZMQ_LINGER, &linger, sizeof (int)));
    ...
    TEST_ASSERT_EQUAL_INT (
      addr_size, TEST_ASSERT_SUCCESS_ERRNO (
                   zmq_send (router_socket, addr, addr_size, SZMQ_SNDMORE)));
    TEST_ASSERT_EQUAL_INT (0, TEST_ASSERT_SUCCESS_ERRNO (zmq_send (
                                router_socket, bottom, 0, SZMQ_SNDMORE)));
