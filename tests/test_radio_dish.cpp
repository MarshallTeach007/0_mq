...
{
    void *dish = test_context_socket (SZMQ_DISH);
...
void test_long_group ()
{
    ...
    void *radio = test_context_socket (SZMQ_RADIO);
...
void test_join_too_long_fails ()
{
    ...
    char too_long_group[SZMQ_GROUP_MAX_LENGTH + 2];
    for (int index = 0; index < SZMQ_GROUP_MAX_LENGTH + 2; index++)
        too_long_group[index] = 'A';
    too_long_group[SZMQ_GROUP_MAX_LENGTH + 1] = '\0';
...
void test_radio_dish_tcp_poll (int ipv6_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (dish, SZMQ_IPV6, &ipv6_, sizeof (int)));
    ...
    zmq_pollitem_t items[] = {
      {radio, 0, SZMQ_POLLIN, 0}, // read publications
      {dish, 0, SZMQ_POLLIN, 0},  // read subscriptions
    };
    int rc = zmq_poll (items, 2, 2000);
    TEST_ASSERT_EQUAL_INT (1, rc);
    TEST_ASSERT_EQUAL_INT (SZMQ_POLLIN, items[1].revents);
...
void test_radio_bind_fails (int ipv6_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (radio, SZMQ_IPV6, &ipv6_, sizeof (int)));
...
static void test_radio_dish_no_loop (int ipv6_)
{
#ifdef _WIN32
    TEST_IGNORE_MESSAGE (
      "SZMQ_MULTICAST_LOOP=false does not appear to work on Windows (TODO)");
#endif
    ignore_if_unavailable (ipv6_);
    ...
    int loop = 0;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (radio, SZMQ_MULTICAST_LOOP, &loop, sizeof (int)));
    ...
    TEST_ASSERT_FAILURE_ERRNO (EAGAIN, zmq_recv (dish, NULL, 0, SZMQ_DONTWAIT));
