...
#ifdef SZMQ_DISCONNECT_MSG
#define LAST_OPTION SZMQ_DISCONNECT_MSG
#else
#define LAST_OPTION SZMQ_BINDTODEVICE
#endif
...
extern "C" int LLVMFuzzerTestOneInput (const uint8_t *data, size_t size)
{
    ...
    void *server = zmq_socket (ctx, SZMQ_XPUB);
    TEST_ASSERT_NOT_NULL (server);

    if (!size)
        return 0;

    for (option = SZMQ_AFFINITY; option <= LAST_OPTION; ++option) {
        uint8_t out[8192];
