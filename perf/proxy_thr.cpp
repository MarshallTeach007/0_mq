...
/*
   Asynchronous proxy benchmark using SZMQ_XPUB_NODROP.
...
static void set_hwm (void *skt)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (skt, SZMQ_SNDHWM, &hwm, sizeof (hwm)));

    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (skt, SZMQ_RCVHWM, &hwm, sizeof (hwm)));
}
...
static void publisher_thread_main (void *pvoid)
{
    ...
    optval = 1;
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_setsockopt (pubsocket, SZMQ_SNDTIMEO, &optval, sizeof (optval)));
...
int main (int argc, char *argv[])
{
    ...
    int rv = zmq_ctx_set (context, SZMQ_IO_THREADS, 4);
