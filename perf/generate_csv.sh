...
#    export REMOTE_LIBSZMQ_PATH="/home/fmontorsi/libzmq/perf"
#    ./generate_csv.sh
#
...
REMOTE_LIBSZMQ_PATH=${REMOTE_LIBSZMQ_PATH:-/root/libzmq/perf}
...
function verify_ssh()
{
    ...
    ssh $REMOTE_IP_SSH "ls $REMOTE_LIBSZMQ_PATH" >/dev/null
    if [ $? -ne 0 ]; then
        echo "The folder $REMOTE_LIBSZMQ_PATH is not valid. Please fix the problem and retry."
        exit 2
    fi
...
function run_remote_perf_util()
{
    ...
    ssh $REMOTE_IP_SSH "$REMOTE_LIBSZMQ_PATH/$REMOTE_PERF_UTIL $TEST_ENDPOINT $MESSAGE_SIZE_BYTES $NUM_MESSAGES" &
    if [ $? -ne 0 ]; then
