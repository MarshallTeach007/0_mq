...
#if defined SZMQ_POLL_BASED_ON_POLL && !defined SZMQ_HAVE_WINDOWS                \
  && !defined SZMQ_HAVE_AIX
#include <poll.h>
#endif
...
capture(class zmq::socket_base_t *capture_, zmq::msg_t *msg_, int more_ = 0) {
    ...
    if (capture_) {
        ...
        if (unlikely (rc < 0))
            return -1;
        rc = capture_->send(&ctrl, more_ ? SZMQ_SNDMORE : 0);
        if (unlikely (rc < 0))
            return -1;
    }
    return 0;
...
static int forward(class zmq::socket_base_t *from_,
    ...
    for (unsigned int i = 0; i < zmq::proxy_burst_size; i++) {
        ...
        while (true) {
            int rc = from_->recv(msg_, SZMQ_DONTWAIT);
            if (rc < 0) {
            ...
            moresz = sizeof more;
            rc = from_->getsockopt(SZMQ_RCVMORE, &more, &moresz);
            if (unlikely (rc < 0))
                return -1;
            ...
            rc = to_->send(msg_, more ? SZMQ_SNDMORE : 0);
            if (unlikely (rc < 0))
                return -1;
...
static int loop_and_send_multipart_stat(zmq::socket_base_t *control_,
    ...
    do {
        rc = control_->send(&msg, more_ ? SZMQ_SNDMORE : 0);
                ...
                class socket_base_t *control_)
{
    ...
    zmq::socket_poller_t *poller_in = new (std::nothrow) zmq::
      socket_poller_t; //  Poll only 'SZMQ_POLLIN' on all sockets. Initial blocking poll in loop.
    zmq::socket_poller_t *poller_control = new (std::nothrow) zmq::
      socket_poller_t; //  Poll only for 'SZMQ_POLLIN' on 'control_', when proxy is paused.
    zmq::socket_poller_t *poller_receive_blocked = new (std::nothrow)
      zmq::socket_poller_t; //  All except 'SZMQ_POLLIN' on 'frontend_'.

    //  If frontend_==backend_ 'poller_send_blocked' and 'poller_receive_blocked' are the same, 'SZMQ_POLLIN' is ignored.
    ...
    zmq::socket_poller_t *poller_send_blocked =
      NULL; //  All except 'SZMQ_POLLIN' on 'backend_'.
    zmq::socket_poller_t *poller_both_blocked =
      NULL; //  All except 'SZMQ_POLLIN' on both 'frontend_' and 'backend_'.
    zmq::socket_poller_t *poller_frontend_only =
      NULL; //  Only 'SZMQ_POLLIN' and 'SZMQ_POLLOUT' on 'frontend_'.
    zmq::socket_poller_t *poller_backend_only =
      NULL; //  Only 'SZMQ_POLLIN' and 'SZMQ_POLLOUT' on 'backend_'.

    if (frontend_ != backend_) {
        poller_send_blocked = new (std::nothrow)
          zmq::socket_poller_t; //  All except 'SZMQ_POLLIN' on 'backend_'.
        poller_both_blocked = new (std::nothrow) zmq::
          socket_poller_t; //  All except 'SZMQ_POLLIN' on both 'frontend_' and 'backend_'.
        poller_frontend_only = new (std::nothrow) zmq::
          socket_poller_t; //  Only 'SZMQ_POLLIN' and 'SZMQ_POLLOUT' on 'frontend_'.
        poller_backend_only = new (std::nothrow) zmq::
          socket_poller_t; //  Only 'SZMQ_POLLIN' and 'SZMQ_POLLOUT' on 'backend_'.
    ...
    zmq::socket_poller_t *poller_wait =
      poller_in; //  Poller for blocking wait, initially all 'SZMQ_POLLIN'.

    //  Register 'frontend_' and 'backend_' with pollers.
    rc = poller_all->add (frontend_, NULL,
                          SZMQ_POLLIN | SZMQ_POLLOUT); //  Everything.
    CHECK_RC_EXIT_ON_FAILURE ();
    rc = poller_in->add (frontend_, NULL, SZMQ_POLLIN); //  All 'SZMQ_POLLIN's.
    CHECK_RC_EXIT_ON_FAILURE ();
    ...
    if (frontend_equal_to_backend) {
        ...
        rc = poller_receive_blocked->add (frontend_, NULL, SZMQ_POLLOUT);
        CHECK_RC_EXIT_ON_FAILURE ();
    } else {
        rc = poller_all->add (backend_, NULL,
                              SZMQ_POLLIN | SZMQ_POLLOUT); //  Everything.
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_in->add (backend_, NULL, SZMQ_POLLIN); //  All 'SZMQ_POLLIN's.
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_both_blocked->add (
          frontend_, NULL, SZMQ_POLLOUT); //  Waiting only for 'SZMQ_POLLOUT'.
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_both_blocked->add (
          backend_, NULL, SZMQ_POLLOUT); //  Waiting only for 'SZMQ_POLLOUT'.
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_send_blocked->add (
          backend_, NULL,
          SZMQ_POLLOUT); //  All except 'SZMQ_POLLIN' on 'backend_'.
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_send_blocked->add (
          frontend_, NULL,
          SZMQ_POLLIN | SZMQ_POLLOUT); //  All except 'SZMQ_POLLIN' on 'backend_'.
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_receive_blocked->add (
          frontend_, NULL,
          SZMQ_POLLOUT); //  All except 'SZMQ_POLLIN' on 'frontend_'.
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_receive_blocked->add (
          backend_, NULL,
          SZMQ_POLLIN | SZMQ_POLLOUT); //  All except 'SZMQ_POLLIN' on 'frontend_'.
        CHECK_RC_EXIT_ON_FAILURE ();
        rc =
          poller_frontend_only->add (frontend_, NULL, SZMQ_POLLIN | SZMQ_POLLOUT);
        CHECK_RC_EXIT_ON_FAILURE ();
        rc =
          poller_backend_only->add (backend_, NULL, SZMQ_POLLIN | SZMQ_POLLOUT);
        CHECK_RC_EXIT_ON_FAILURE ();
    }

    //  Register 'control_' with pollers.
    if (control_ != NULL) {
        rc = poller_all->add (control_, NULL, SZMQ_POLLIN);
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_in->add (control_, NULL, SZMQ_POLLIN);
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_control->add (
          control_, NULL,
          SZMQ_POLLIN); //  When proxy is paused we wait only for SZMQ_POLLIN on 'control_' socket.
        CHECK_RC_EXIT_ON_FAILURE ();
        rc = poller_receive_blocked->add (control_, NULL, SZMQ_POLLIN);
        CHECK_RC_EXIT_ON_FAILURE ();
        if (!frontend_equal_to_backend) {
            rc = poller_send_blocked->add (control_, NULL, SZMQ_POLLIN);
            CHECK_RC_EXIT_ON_FAILURE ();
            rc = poller_both_blocked->add (control_, NULL, SZMQ_POLLIN);
            CHECK_RC_EXIT_ON_FAILURE ();
            rc = poller_frontend_only->add (control_, NULL, SZMQ_POLLIN);
            CHECK_RC_EXIT_ON_FAILURE ();
            rc = poller_backend_only->add (control_, NULL, SZMQ_POLLIN);
            CHECK_RC_EXIT_ON_FAILURE ();
        }
    }
    ...
    while (state != terminated) {
        //  Blocking wait initially only for 'SZMQ_POLLIN' - 'poller_wait' points to 'poller_in'.
        //  If one of receiving end's queue is full ('SZMQ_POLLOUT' not available),
        ...
        for (int i = 0; i < rc; i++) {
            if (events[i].socket == frontend_) {
                frontend_in = (events[i].events & SZMQ_POLLIN) != 0;
                frontend_out = (events[i].events & SZMQ_POLLOUT) != 0;
            } else
              ...
              if (events[i].socket == backend_) {
                backend_in = (events[i].events & SZMQ_POLLIN) != 0;
                backend_out = (events[i].events & SZMQ_POLLOUT) != 0;
            } else if (events[i].socket == control_)
                control_in = (events[i].events & SZMQ_POLLIN) != 0;
        }
        ...
        if (control_in) {
            ...
            CHECK_RC_EXIT_ON_FAILURE ();
            rc = control_->getsockopt (SZMQ_RCVMORE, &more, &moresz);
        ...
        if (state == active) {
            //  Process a request, 'SZMQ_POLLIN' on 'frontend_' and 'SZMQ_POLLOUT' on 'backend_'.
            //  In case of frontend_==backend_ there's no 'SZMQ_POLLOUT' event.
            ...
            //  Process a reply, 'SZMQ_POLLIN' on 'backend_' and 'SZMQ_POLLOUT' on 'frontend_'.
            ...
            if (request_processed || reply_processed) {
                //  If request/reply is processed that means we had at least one 'SZMQ_POLLOUT' event.
                //  Enable corresponding 'SZMQ_POLLIN' for blocking wait if any was disabled.
                ...
                //  No requests have been processed, there were no 'SZMQ_POLLIN' with corresponding 'SZMQ_POLLOUT' events.
                //  That means that out queue(s) is/are full or one out queue is full and second one has no messages to process.
                //  Disable receiving 'SZMQ_POLLIN' for sockets for which there's no 'SZMQ_POLLOUT',
                //  or wait only on both 'backend_''s or 'frontend_''s 'SZMQ_POLLIN' and 'SZMQ_POLLOUT'.
                if (frontend_in) {
                    if (frontend_out)
                        // If frontend_in and frontend_out are true, obviously backend_in and backend_out are both false.
                        // In that case we need to wait for both 'SZMQ_POLLIN' and 'SZMQ_POLLOUT' only on 'backend_'.
                ...
                if (backend_in) {
                    ...
                    if (backend_out)
                        ...
                        // In that case we need to wait for both 'SZMQ_POLLIN' and 'SZMQ_POLLOUT' only on 'frontend_'.
                        poller_wait = poller_frontend_only;
                    else {
...
int zmq::proxy(class socket_base_t *frontend_,
    ...
    int more;
    size_t moresz;
    zmq_pollitem_t items[] = {{frontend_, 0, SZMQ_POLLIN, 0},
                              {backend_,  0, SZMQ_POLLIN, 0},
                              {control_,  0, SZMQ_POLLIN, 0}};
    int qt_poll_items = (control_ ? 3 : 2);
    zmq_pollitem_t itemsout[] = {{frontend_, 0, SZMQ_POLLOUT, 0},
                                 {backend_,  0, SZMQ_POLLOUT, 0}};
    ...
    while (state != terminated) {
        ...
        //  Process a control command if any
        if (control_ && items[2].revents & SZMQ_POLLIN) {
            rc = control_->recv(&msg, 0);
            if (unlikely (rc < 0))
                return close_and_return(&msg, -1);
        ...
        //  Process a request
        if (state == active && items[0].revents & SZMQ_POLLIN
            && (frontend_ == backend_ || itemsout[1].revents & SZMQ_POLLOUT)) {
        ...
        if (state == active && frontend_ != backend_
            && items[1].revents & SZMQ_POLLIN
            && itemsout[0].revents & SZMQ_POLLOUT) {
