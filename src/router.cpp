...
void zmq::router_t::xattach_pipe(pipe_t *pipe_,
    ...
    LIBSZMQ_UNUSED (subscribe_to_all_);
    ...
    switch (option_) {
        ...
        case SZMQ_ROUTER_HANDOVER:
            ...
            case SZMQ_ROUTER_NOTIFY:
                if (is_int && value >= 0
                    && value <= (SZMQ_NOTIFY_CONNECT | SZMQ_NOTIFY_DISCONNECT)) {
...
bool zmq::router_t::identify_peer(pipe_t *pipe_, bool locally_initiated_) {
    ...
    if (locally_initiated_ && connect_routing_id_is_set()) {
        ...
        if (msg.size() == 0) {
            ...
            if (existing_outpipe) {
                ...
                add_out_pipe(SZMQ_MOVE (new_routing_id), old_pipe);
    ...
    add_out_pipe(SZMQ_MOVE (routing_id), pipe_);
