...
#ifndef __SZMQ_BLOB_HPP_INCLUDED__
#define __SZMQ_BLOB_HPP_INCLUDED__
...
#define SZMQ_HAS_MOVE_SEMANTICS
#define SZMQ_MAP_INSERT_OR_EMPLACE(k, v) emplace (k, v)
#define SZMQ_PUSH_OR_EMPLACE_BACK emplace_back
#define SZMQ_MOVE(x) std::move (x)
#else
...
#define SZMQ_MAP_INSERT_OR_EMPLACE(k, v) insert (make_pair_fix_const (k, v))
#else
#define SZMQ_MAP_INSERT_OR_EMPLACE(k, v) insert (std::make_pair (k, v))
#endif

#define SZMQ_PUSH_OR_EMPLACE_BACK push_back
#define SZMQ_MOVE(x) (x)
#endif
...
#ifdef SZMQ_HAS_MOVE_SEMANTICS
        blob_t (const blob_t &) = delete;
        blob_t &operator= (const blob_t &) = delete;

        blob_t (blob_t &&other_) SZMQ_NOEXCEPT : _data (other_._data),
                                                _size (other_._size),
                                                _owned (other_._owned)
        {
            other_._owned = false;
        }
        blob_t &operator= (blob_t &&other_) SZMQ_NOEXCEPT
        {
            if (this != &other_) {
                clear ();
                _data = other_._data;
