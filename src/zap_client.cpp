...
namespace zmq {
    ...
    int zap_client_t::receive_and_process_zap_reply() {
        ...
        for (size_t i = 0; i < zap_reply_frame_count; i++) {
            ...
            if ((msg[i].flags() & msg_t::more)
                ...
                session->get_socket()->event_handshake_failed_protocol(
                        session->get_endpoint(), SZMQ_PROTOCOL_ERROR_ZAP_MALFORMED_REPLY);
                errno = EPROTO;
                return close_and_return(msg, -1);
            }
        }
        ...
        if (msg[0].size() > 0) {
            ...
            session->get_socket()->event_handshake_failed_protocol(
                    session->get_endpoint(), SZMQ_PROTOCOL_ERROR_ZAP_UNSPECIFIED);
            errno = EPROTO;
            return close_and_return(msg, -1);
        }
        ...
        if (msg[1].size() != zap_version_len
            ...
            session->get_socket()->event_handshake_failed_protocol(
                    session->get_endpoint(), SZMQ_PROTOCOL_ERROR_ZAP_BAD_VERSION);
            errno = EPROTO;
            return close_and_return(msg, -1);
        }
        ...
        if (msg[2].size() != id_len || memcmp(msg[2].data(), id, id_len)) {
            session->get_socket()->event_handshake_failed_protocol(
                    session->get_endpoint(), SZMQ_PROTOCOL_ERROR_ZAP_BAD_REQUEST_ID);
            errno = EPROTO;
            return close_and_return(msg, -1);
        }
        ...
        if (msg[3].size() != 3 || status_code_data[0] < '2'
            ...
            session->get_socket()->event_handshake_failed_protocol(
                    session->get_endpoint(), SZMQ_PROTOCOL_ERROR_ZAP_INVALID_STATUS_CODE);
            errno = EPROTO;
            return close_and_return(msg, -1);
        }
        ...
        if (rc != 0) {
            session->get_socket()->event_handshake_failed_protocol(
                    session->get_endpoint(), SZMQ_PROTOCOL_ERROR_ZAP_INVALID_METADATA);
            errno = EPROTO;
            return close_and_return(msg, -1);
        }
