...
zmq::xpub_t::~xpub_t() {
    ...
    for (std::deque<metadata_t *>::iterator it = _pending_metadata.begin(),
         ...
         it != end; ++it)
        if (*it && (*it)->drop_ref()) LIBSZMQ_DELETE (*it);
}

void zmq::xpub_t::xattach_pipe(pipe_t *pipe_,
                               bool subscribe_to_all_,
                               bool locally_initiated_) {
    LIBSZMQ_UNUSED (locally_initiated_);
...
void zmq::xpub_t::xread_activated(pipe_t *pipe_) {
    ...
    while (pipe_->read(&msg)) {
        ...
        if (is_subscribe_or_cancel) {
            ...
            if (_manual || (options.type == SZMQ_XPUB && notify)) {
                ...
                _pending_data.push_back(SZMQ_MOVE (notification));
                if (metadata)
                    metadata->add_ref();
                _pending_metadata.push_back(metadata);
                _pending_flags.push_back(0);
            }
        } else if (options.type != SZMQ_PUB) {
...
int zmq::xpub_t::xsetsockopt(int option_,
                             const void *optval_,
                             size_t optvallen_) {
    if (option_ == SZMQ_XPUB_VERBOSE || option_ == SZMQ_XPUB_VERBOSER
        || option_ == SZMQ_XPUB_MANUAL_LAST_VALUE || option_ == SZMQ_XPUB_NODROP
        || option_ == SZMQ_XPUB_MANUAL || option_ == SZMQ_ONLY_FIRST_SUBSCRIBE) {
        if (optvallen_ != sizeof(int)
            || *static_cast<const int *> (optval_) < 0) {
            errno = EINVAL;
            return -1;
        }
        if (option_ == SZMQ_XPUB_VERBOSE) {
            _verbose_subs = (*static_cast<const int *> (optval_) != 0);
            _verbose_unsubs = false;
        } else if (option_ == SZMQ_XPUB_VERBOSER) {
            _verbose_subs = (*static_cast<const int *> (optval_) != 0);
            _verbose_unsubs = _verbose_subs;
        } else if (option_ == SZMQ_XPUB_MANUAL_LAST_VALUE) {
            _manual = (*static_cast<const int *> (optval_) != 0);
            _send_last_pipe = _manual;
        } else if (option_ == SZMQ_XPUB_NODROP)
            _lossy = (*static_cast<const int *> (optval_) == 0);
        else if (option_ == SZMQ_XPUB_MANUAL)
            _manual = (*static_cast<const int *> (optval_) != 0);
        else if (option_ == SZMQ_ONLY_FIRST_SUBSCRIBE)
            _only_first_subscribe = (*static_cast<const int *> (optval_) != 0);
    } else if (option_ == SZMQ_SUBSCRIBE && _manual) {
        if (_last_pipe != NULL)
            _subscriptions.add((unsigned char *) optval_, optvallen_,
                               _last_pipe);
    } else if (option_ == SZMQ_UNSUBSCRIBE && _manual) {
        if (_last_pipe != NULL)
            _subscriptions.rm((unsigned char *) optval_, optvallen_,
                              _last_pipe);
    } else if (option_ == SZMQ_XPUB_WELCOME_MSG) {
        _welcome_msg.close();
...
static void stub(zmq::mtrie_t::prefix_t data_, size_t size_, void *arg_) {
    LIBSZMQ_UNUSED (data_);
    LIBSZMQ_UNUSED (size_);
    LIBSZMQ_UNUSED (arg_);
}
...
void zmq::xpub_t::send_unsubscription(zmq::mtrie_t::prefix_t data_,
                                      size_t size_,
                                      xpub_t *self_) {
    if (self_->options.type != SZMQ_PUB) {
        ...
        *unsub.data() = 0;
        ...
        self_->_pending_data.SZMQ_PUSH_OR_EMPLACE_BACK(SZMQ_MOVE (unsub));
