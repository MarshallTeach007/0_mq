...
void zmq::socket_base_t::inprocs_t::emplace(const char *endpoint_uri_,
                                            pipe_t *pipe_) {
    _inprocs.SZMQ_MAP_INSERT_OR_EMPLACE (std::string(endpoint_uri_), pipe_);
}
...
zmq::socket_base_t::socket_base_t(ctx_t *parent_,
    ...
    options.ipv6 = (parent_->get(SZMQ_IPV6) != 0);
    options.linger.store(parent_->get(SZMQ_BLOCKY) ? -1 : 0);
    options.zero_copy = parent_->get(SZMQ_ZERO_COPY_RECV) != 0;
...
int zmq::socket_base_t::get_peer_state(const void *routing_id_,
    ...
    LIBSZMQ_UNUSED (routing_id_);
    LIBSZMQ_UNUSED (routing_id_size_);
...
zmq::socket_base_t::~socket_base_t() {
    if (_mailbox) LIBSZMQ_DELETE (_mailbox);

    if (_reaper_signaler) LIBSZMQ_DELETE (_reaper_signaler);
...
int zmq::socket_base_t::check_protocol(const std::string &protocol_) const {
...
#if defined SZMQ_HAVE_OPENPGM || defined SZMQ_HAVE_NORM
#if defined SZMQ_HAVE_OPENPGM && defined SZMQ_HAVE_NORM
    if ((protocol_ == protocol_name::pgm || protocol_ == protocol_name::epgm
         || protocol_ == protocol_name::norm)
#elif defined SZMQ_HAVE_OPENPGM
    if ((protocol_ == protocol_name::pgm || protocol_ == protocol_name::epgm)
#else // defined SZMQ_HAVE_NORM
    if (protocol_ == protocol_name::norm
#endif
        && options.type != SZMQ_PUB && options.type != SZMQ_SUB
        && options.type != SZMQ_XPUB && options.type != SZMQ_XSUB) {
    ...
    if (protocol_ == protocol_name::udp
        && (options.type != SZMQ_DISH && options.type != SZMQ_RADIO
            && options.type != SZMQ_DGRAM)) {
        errno = ENOCOMPATPROTO;
        return -1;
    }
...
int zmq::socket_base_t::getsockopt(int option_,
    ...
    if (option_ == SZMQ_EVENTS) {
        ...
        return do_getsockopt<int>(optval_, optvallen_,
                                  (has_out() ? SZMQ_POLLOUT : 0)
                                  | (has_in() ? SZMQ_POLLIN : 0));
    }

    if (option_ == SZMQ_LAST_ENDPOINT) {
        return do_getsockopt(optval_, optvallen_, _last_endpoint);
    }

    if (option_ == SZMQ_THREAD_SAFE) {
...
int zmq::socket_base_t::bind(const char *endpoint_uri_) {
    ...
    if (protocol == protocol_name::udp) {
        if (!(options.type == SZMQ_DGRAM || options.type == SZMQ_DISH)) {
            errno = ENOCOMPATPROTO;
            return -1;
        }
...
int zmq::socket_base_t::connect_internal(const char *endpoint_uri_) {
    ...
    const bool is_single_connect =
            (options.type == SZMQ_DEALER || options.type == SZMQ_SUB
             || options.type == SZMQ_PUB || options.type == SZMQ_REQ);
    ...
    // TBD - Should we check address for SZMQ_HAVE_NORM???
...
void zmq::socket_base_t::add_endpoint(
    ...
    _endpoints.SZMQ_MAP_INSERT_OR_EMPLACE (endpoint_pair_.identifier(),
...
int zmq::socket_base_t::term_endpoint(const char *endpoint_uri_) {
    ...
    if (options.reconnect_stop & SZMQ_RECONNECT_STOP_AFTER_DISCONNECT) {
        _disconnected = true;
    }
...
int zmq::socket_base_t::send(msg_t *msg_, int flags_) {
    ...
    //  Special case for SZMQ_PUSH: -2 means pipe is dead while a
    ...
    if (unlikely (rc == -2)) {
        if (!((flags_ & SZMQ_DONTWAIT) || options.sndtimeo == 0)) {
            rc = msg_->close();
            errno_assert (rc == 0);
            rc = msg_->init();
            errno_assert (rc == 0);
...
int zmq::socket_base_t::recv(msg_t *msg_, int flags_) {
    ...
    if ((flags_ & SZMQ_DONTWAIT) || options.rcvtimeo == 0) {
        if (unlikely (process_commands(0, false) != 0)) {
            return -1;
        }
        _ticks = 0;
...
void zmq::socket_base_t::process_pipe_stats_publish(
    ...
    event(*endpoint_pair_, values, 2, SZMQ_EVENT_PIPES_STATS);
...
int zmq::socket_base_t::query_pipes_stats() {
    {
        scoped_lock_t lock(_monitor_sync);
        if (!(_monitor_events & SZMQ_EVENT_PIPES_STATS)) {
            errno = EINVAL;
            return -1;
        }
    }
...
void zmq::socket_base_t::update_pipe_options(int option_) {
    if (option_ == SZMQ_SNDHWM || option_ == SZMQ_RCVHWM) {
        for (pipes_t::size_type i = 0, size = _pipes.size(); i != size; ++i) {
...
int zmq::socket_base_t::monitor(const char *endpoint_,
    ...
    int linger = 0;
    int rc =
            zmq_setsockopt(_monitor_socket, SZMQ_LINGER, &linger, sizeof(linger));
    if (rc == -1)
        stop_monitor(false);
...
void zmq::socket_base_t::event_connected(
    ...
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_CONNECTED);
}
...
void zmq::socket_base_t::event_connect_delayed(
    ...
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_CONNECT_DELAYED);
}
...
void zmq::socket_base_t::event_connect_retried(
    ...
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_CONNECT_RETRIED);
}

void zmq::socket_base_t::event_listening(
        const endpoint_uri_pair_t &endpoint_uri_pair_, zmq::fd_t fd_) {
    uint64_t values[1] = {static_cast<uint64_t> (fd_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_LISTENING);
}

void zmq::socket_base_t::event_bind_failed(
        const endpoint_uri_pair_t &endpoint_uri_pair_, int err_) {
    uint64_t values[1] = {static_cast<uint64_t> (err_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_BIND_FAILED);
}

void zmq::socket_base_t::event_accepted(
        const endpoint_uri_pair_t &endpoint_uri_pair_, zmq::fd_t fd_) {
    uint64_t values[1] = {static_cast<uint64_t> (fd_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_ACCEPTED);
}

void zmq::socket_base_t::event_accept_failed(
        const endpoint_uri_pair_t &endpoint_uri_pair_, int err_) {
    uint64_t values[1] = {static_cast<uint64_t> (err_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_ACCEPT_FAILED);
}

void zmq::socket_base_t::event_closed(
        const endpoint_uri_pair_t &endpoint_uri_pair_, zmq::fd_t fd_) {
    uint64_t values[1] = {static_cast<uint64_t> (fd_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_CLOSED);
}

void zmq::socket_base_t::event_close_failed(
        const endpoint_uri_pair_t &endpoint_uri_pair_, int err_) {
    uint64_t values[1] = {static_cast<uint64_t> (err_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_CLOSE_FAILED);
}

void zmq::socket_base_t::event_disconnected(
        const endpoint_uri_pair_t &endpoint_uri_pair_, zmq::fd_t fd_) {
    uint64_t values[1] = {static_cast<uint64_t> (fd_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_DISCONNECTED);
}

void zmq::socket_base_t::event_handshake_failed_no_detail(
        const endpoint_uri_pair_t &endpoint_uri_pair_, int err_) {
    uint64_t values[1] = {static_cast<uint64_t> (err_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_HANDSHAKE_FAILED_NO_DETAIL);
}

void zmq::socket_base_t::event_handshake_failed_protocol(
        const endpoint_uri_pair_t &endpoint_uri_pair_, int err_) {
    uint64_t values[1] = {static_cast<uint64_t> (err_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_HANDSHAKE_FAILED_PROTOCOL);
}

void zmq::socket_base_t::event_handshake_failed_auth(
        const endpoint_uri_pair_t &endpoint_uri_pair_, int err_) {
    uint64_t values[1] = {static_cast<uint64_t> (err_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_HANDSHAKE_FAILED_AUTH);
}

void zmq::socket_base_t::event_handshake_succeeded(
        const endpoint_uri_pair_t &endpoint_uri_pair_, int err_) {
    uint64_t values[1] = {static_cast<uint64_t> (err_)};
    event(endpoint_uri_pair_, values, 1, SZMQ_EVENT_HANDSHAKE_SUCCEEDED);
}
    ...
    if (_monitor_socket) {
        ...
        switch (options.monitor_event_version) {
            case 1: {
                ...
                zmq_msg_send(&msg, _monitor_socket, SZMQ_SNDMORE);
...
void zmq::socket_base_t::stop_monitor(bool send_monitor_stopped_event_) {
    ...
    if (_monitor_socket) {
        if ((_monitor_events & SZMQ_EVENT_MONITOR_STOPPED)
            && send_monitor_stopped_event_) {
            uint64_t values[1] = {0};
            monitor_event(SZMQ_EVENT_MONITOR_STOPPED, values, 1,
                          endpoint_uri_pair_t());
        }
...
std::string zmq::routing_socket_base_t::extract_connect_routing_id() {
    std::string res = SZMQ_MOVE (_connect_routing_id);
...
void zmq::routing_socket_base_t::add_out_pipe(blob_t routing_id_,
    ...
    const bool ok =
            _out_pipes.SZMQ_MAP_INSERT_OR_EMPLACE (SZMQ_MOVE(routing_id_), outpipe)
                    .second;
    zmq_assert (ok);
}
