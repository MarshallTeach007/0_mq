...
#ifndef __SZMQ_GENERIC_MTRIE_IMPL_HPP_INCLUDED__
#define __SZMQ_GENERIC_MTRIE_IMPL_HPP_INCLUDED__
...
namespace zmq {
    ...
    template<typename T>
    generic_mtrie_t<T>::~generic_mtrie_t() {
        LIBSZMQ_DELETE (_pipes);

        if (_count == 1) {
            zmq_assert (_next.node);
            LIBSZMQ_DELETE (_next.node);
        } else if (_count > 1) {
            for (unsigned short i = 0; i != _count; ++i) {
                LIBSZMQ_DELETE (_next.table[i]);
            }
            free(_next.table);
        }
    }
                ...
                switch (it.node->_count) {
                    ...
                    case 1:
                        ...
                        if (it.node->_next.node->is_redundant()) {
                            LIBSZMQ_DELETE (it.node->_next.node);
                            it.node->_count = 0;
                            --it.node->_live_nodes;
    ...
    template<typename T>
    bool generic_mtrie_t<T>::is_redundant() const {
