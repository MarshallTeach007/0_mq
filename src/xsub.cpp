...
int zmq::xsub_t::xsetsockopt(int option_,
                             const void *optval_,
                             size_t optvallen_) {
    if (option_ == SZMQ_ONLY_FIRST_SUBSCRIBE) {
    ...
    else if (option_ == SZMQ_XSUB_VERBOSE_UNSUBSCRIBE) {
...
int zmq::xsub_t::xsend(msg_t *msg_) {
    ...
    if (msg_->is_subscribe() || (size > 0 && *data == 1)) {
        ...
        //  doing it here as well breaks SZMQ_XPUB_VERBOSE
...
void zmq::xsub_t::send_subscription(unsigned char *data_,
    ...
    //  zmq_setsockopt(SZMQ_SUBSCRIBE, ...), which also drops subscriptions
