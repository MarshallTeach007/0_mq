...
#ifndef __SZMQ_PIPE_HPP_INCLUDED__
#define __SZMQ_PIPE_HPP_INCLUDED__
...
//  read (older messages are discarded)
    ...
    struct i_pipe_events {
        virtual ~i_pipe_events() SZMQ_DEFAULT;
...
//  the generic array of pipes to be deallocated (3).
    ...
    class pipe_t SZMQ_FINAL : public object_t,
    ...
    private:
        ...
        //  Command handlers.
        void process_activate_read() SZMQ_OVERRIDE;

        void process_activate_write(uint64_t msgs_read_) SZMQ_OVERRIDE;

        void process_hiccup(void *pipe_) SZMQ_OVERRIDE;

        void
        process_pipe_peer_stats(uint64_t queue_count_,
                                own_t *socket_base_,
                                endpoint_uri_pair_t *endpoint_pair_) SZMQ_OVERRIDE;

        void process_pipe_term() SZMQ_OVERRIDE;

        void process_pipe_term_ack() SZMQ_OVERRIDE;

        void process_pipe_hwm(int inhwm_, int outhwm_) SZMQ_OVERRIDE;
    ...
    SZMQ_NON_COPYABLE_NOR_MOVABLE (pipe_t)
    };
