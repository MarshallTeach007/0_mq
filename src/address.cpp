...
zmq::address_t::~address_t() {
    if (protocol == protocol_name::tcp) {
        LIBSZMQ_DELETE (resolved.tcp_addr);
    } else if (protocol == protocol_name::udp) {
        LIBSZMQ_DELETE (resolved.udp_addr);
    }
#ifdef SZMQ_HAVE_WS
    else if (protocol == protocol_name::ws) {
        LIBSZMQ_DELETE (resolved.ws_addr);
    }
#endif
        ...
        LIBSZMQ_DELETE (resolved.ipc_addr);
    }
#endif
#if defined SZMQ_HAVE_TIPC
    else if (protocol == protocol_name::tipc) {
        LIBSZMQ_DELETE (resolved.tipc_addr);
    }
#endif
#if defined SZMQ_HAVE_VMCI
    else if (protocol == protocol_name::vmci) {
        LIBSZMQ_DELETE (resolved.vmci_addr);
    }
#endif
}
