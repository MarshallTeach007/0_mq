...
#define SZMQ_TYPE_UNSAFE

#include "macros.hpp"
#include "poller.hpp"
#include "peer.hpp"

#if !defined SZMQ_HAVE_POLLER
...
#ifdef SZMQ_HAVE_VXWORKS
#include <strings.h>
#endif
#endif

// XSI vector I/O
#if defined SZMQ_HAVE_UIO
#include <sys/uio.h>
#else
struct iovec {
    void *iov_base;
...
void zmq_version(int *major_, int *minor_, int *patch_) {
    *major_ = SZMQ_VERSION_MAJOR;
    *minor_ = SZMQ_VERSION_MINOR;
    *patch_ = SZMQ_VERSION_PATCH;
}
...
void *zmq_init(int io_threads_) {
    if (io_threads_ >= 0) {
        ...
        zmq_ctx_set(ctx, SZMQ_IO_THREADS, io_threads_);
        return ctx;
    }
    errno = EINVAL;
    return NULL;
...
int zmq_socket_monitor(void *s_, const char *addr_, int events_) {
    return zmq_socket_monitor_versioned(s_, addr_, events_, 1, SZMQ_PAIR);
}
...
uint32_t zmq_connect_peer(void *s_, const char *addr_) {
    ...
    if (s->getsockopt(SZMQ_TYPE, &socket_type, &socket_type_size) != 0)
        return 0;

    if (socket_type != SZMQ_PEER) {
        errno = ENOTSUP;
        return 0;
    }
...
s_sendmsg(zmq::socket_base_t *s_, zmq_msg_t *msg_, int flags_) {
    ...
    //  int max_msgsz = s_->parent->get (SZMQ_MAX_MSGSZ);
...
//
// If flag bit SZMQ_SNDMORE is set the vector is treated as
// a single multi-part message, i.e. the last message has
// SZMQ_SNDMORE bit switched off.
//
...
// SZMQ_RCVMORE is set to indicate if a complete multi-part message was read.
    ...
    *count_ = 0;
    ...
    for (size_t i = 0; recvmore && i < count; ++i) {
        ...
        memcpy(a_[i].iov_base, static_cast<char *> (zmq_msg_data(&msg)),
               a_[i].iov_len);
        // Assume zmq_socket SZMQ_RVCMORE is properly set.
...
int zmq_msg_more(const zmq_msg_t *msg_) {
    return zmq_msg_get(msg_, SZMQ_MORE);
}
...
int zmq_poll(zmq_pollitem_t *items_, int nitems_, long timeout_) {
...
#endif // SZMQ_HAVE_POLLER
#if defined SZMQ_POLL_BASED_ON_POLL || defined SZMQ_POLL_BASED_ON_SELECT
    ...
    if (unlikely (nitems_ == 0)) {
        ...
        return 0;
#elif defined SZMQ_HAVE_VXWORKS
        struct timespec ns_;
    ...
    uint64_t now = 0;
    uint64_t end = 0;
#if defined SZMQ_POLL_BASED_ON_POLL
    zmq::fast_vector_t<pollfd, SZMQ_POLLITEMS_DFLT> pollfds (nitems_);

    //  Build pollset for poll () system call.
    for (int i = 0; i != nitems_; i++) {
        //  If the poll item is a 0MQ socket, we poll on the file descriptor
        //  retrieved by the SZMQ_FD socket option.
        if (items_[i].socket) {
            size_t zmq_fd_size = sizeof (zmq::fd_t);
            if (zmq_getsockopt (items_[i].socket, SZMQ_FD, &pollfds[i].fd,
                                &zmq_fd_size)
                == -1) {
                return -1;
            }
        ...
        //  events to normal POLLIN/POLLOUT for poll ().
            ...
            pollfds[i].events =
              (items_[i].events & SZMQ_POLLIN ? POLLIN : 0)
              | (items_[i].events & SZMQ_POLLOUT ? POLLOUT : 0)
              | (items_[i].events & SZMQ_POLLPRI ? POLLPRI : 0);
        }
    }
#else
    ...
    for (int i = 0; i != nitems_; i++) {
        ...
        //  notification file descriptor retrieved by the SZMQ_FD socket option.
        if (items_[i].socket) {
            size_t zmq_fd_size = sizeof (zmq::fd_t);
            zmq::fd_t notify_fd;
            if (zmq_getsockopt (items_[i].socket, SZMQ_FD, &notify_fd,
                                &zmq_fd_size)
                == -1)
                return -1;
            if (items_[i].events) {
        ...
        //  events to the appropriate fd_sets.
            ...
            if (items_[i].events & SZMQ_POLLIN)
                FD_SET (items_[i].fd, pollset_in.get ());
            if (items_[i].events & SZMQ_POLLOUT)
                FD_SET (items_[i].fd, pollset_out.get ());
            if (items_[i].events & SZMQ_POLLERR)
    ...
    while (true) {
        ...
        for (int i = 0; i != nitems_; i++) {
            ...
            //  using the SZMQ_EVENTS socket option.
            if (items_[i].socket) {
                size_t zmq_events_size = sizeof (uint32_t);
                uint32_t zmq_events;
                if (zmq_getsockopt (items_[i].socket, SZMQ_EVENTS, &zmq_events,
                                    &zmq_events_size)
                    == -1) {
                    return -1;
                }
                if ((items_[i].events & SZMQ_POLLOUT)
                    && (zmq_events & SZMQ_POLLOUT))
                    items_[i].revents |= SZMQ_POLLOUT;
                if ((items_[i].events & SZMQ_POLLIN)
                    && (zmq_events & SZMQ_POLLIN))
                    items_[i].revents |= SZMQ_POLLIN;
            }
            ...
            //  the events to zmq_pollitem_t-style format.
                ...
                if (pollfds[i].revents & POLLPRI)
                    items_[i].revents |= SZMQ_POLLPRI;
                if (pollfds[i].revents & ~(POLLIN | POLLOUT | POLLPRI))
                    items_[i].revents |= SZMQ_POLLERR;
            }
    ...
    for (int i = 0; i != nitems_; i++) {
        ...
        if (items_[i].socket) {
            ...
            if (zmq_getsockopt (items_[i].socket, SZMQ_EVENTS, &zmq_events,
                ...
                == -1)
                return -1;
            if ((items_[i].events & SZMQ_POLLOUT) && (zmq_events & SZMQ_POLLOUT))
                items_[i].revents |= SZMQ_POLLOUT;
            if ((items_[i].events & SZMQ_POLLIN) && (zmq_events & SZMQ_POLLIN))
                items_[i].revents |= SZMQ_POLLIN;
        }
        //  Else, the poll item is a raw file descriptor, simply convert
        //  the events to zmq_pollitem_t-style format.
...
static int check_events(const short events_) {
    if (events_ & ~(SZMQ_POLLIN | SZMQ_POLLOUT | SZMQ_POLLERR | SZMQ_POLLPRI)) {
        errno = EINVAL;
        return -1;
    }
    return 0;
...
int zmq_has(const char *capability_) {
    ...
    if (strcmp (capability_, zmq::protocol_name::vmci) == 0)
        return true;
#endif
#if defined(SZMQ_BUILD_DRAFT_API)
    if (strcmp (capability_, "draft") == 0)
        return true;
#endif
#if defined(SZMQ_HAVE_WS)
    if (strcmp (capability_, "WS") == 0)
        return true;
#endif
#if defined(SZMQ_HAVE_WSS)
