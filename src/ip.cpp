...
#if defined SZMQ_HAVE_OPENVMS || defined SZMQ_HAVE_VXWORKS
#include <ioctl.h>
#endif
...
#if defined SZMQ_HAVE_SOCK_CLOEXEC
    type_ |= SOCK_CLOEXEC;
#endif

#if defined SZMQ_HAVE_WINDOWS && defined WSA_FLAG_NO_HANDLE_INHERIT
...
#elif defined SZMQ_HAVE_OPENVMS || defined SZMQ_HAVE_VXWORKS
    int nonblock = 1;
...
#if defined IPV6_V6ONLY && !defined SZMQ_HAVE_OPENBSD                           \
  && !defined SZMQ_HAVE_DRAGONFLY
#ifdef SZMQ_HAVE_WINDOWS
    DWORD flag = 0;
#else
    int flag = 0;
...
#if !defined(SZMQ_HAVE_WINDOWS) && defined(IPV6_TCLASS)
...
#ifdef SZMQ_HAVE_SO_BINDTODEVICE
...
{
#if !defined _WIN32_WCE && !defined SZMQ_HAVE_WINDOWS_UWP
...
#if defined SZMQ_HAVE_EVENTFD
    int flags = 0;
#if defined SZMQ_HAVE_EVENTFD_CLOEXEC
    //  Setting this option result in sane behaviour when exec() functions
...
#elif defined SZMQ_HAVE_OPENVMS
...
#if defined SZMQ_HAVE_WINDOWS && !defined _WIN32_WCE                            \
  && !defined SZMQ_HAVE_WINDOWS_UWP
    //  On Windows, preventing sockets to be inherited by child processes.
    const BOOL brc = SetHandleInformation (reinterpret_cast<HANDLE> (sock_),
                                           HANDLE_FLAG_INHERIT, 0);
    win_assert (brc);
#elif (!defined SZMQ_HAVE_SOCK_CLOEXEC || !defined HAVE_ACCEPT4)                \
 && defined FD_CLOEXEC
