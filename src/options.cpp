...
int zmq::do_setsockopt_int_as_bool_strict(const void *const optval_,
    ...
    // SZMQ_ROUTER_RAW any positive value is accepted
    int value = -1;
...
zmq::options_t::options_t() :
        ...
        mechanism(SZMQ_NULL),
        as_server(0),
        gss_principal_nt(SZMQ_GSSAPI_NT_HOSTBASED),
        gss_service_principal_nt(SZMQ_GSSAPI_NT_HOSTBASED),
        gss_plaintext(false),
        socket_id(0),
        conflate(false),
        handshake_ivl(30000),
...
int zmq::options_t::setsockopt(int option_,
    ...
    switch (option_) {
        ...
        case SZMQ_SNDTIMEO:
            ...
            /*  Deprecated in favor of SZMQ_IPV6  */
        case SZMQ_IPV4ONLY: {
            bool value;
            const int rc =
...
#if defined SZMQ_HAVE_SO_PEERCRED || defined SZMQ_HAVE_LOCAL_PEERCRED
            case SZMQ_IPC_FILTER_UID:
                return do_setsockopt_set (optval_, optvallen_,
                                          &ipc_uid_accept_filters);


            case SZMQ_IPC_FILTER_GID:
                return do_setsockopt_set (optval_, optvallen_,
                                          &ipc_gid_accept_filters);
#endif

#if defined SZMQ_HAVE_SO_PEERCRED
            case SZMQ_IPC_FILTER_PID:
                return do_setsockopt_set (optval_, optvallen_,
                                          &ipc_pid_accept_filters);
#endif

        case SZMQ_PLAIN_SERVER:
            if (is_int && (value == 0 || value == 1)) {
                as_server = value;
                mechanism = value ? SZMQ_PLAIN : SZMQ_NULL;
                return 0;
            }
            break;

        case SZMQ_PLAIN_USERNAME:
            if (optvallen_ == 0 && optval_ == NULL) {
                mechanism = SZMQ_NULL;
                return 0;
                ...
                plain_username.assign(static_cast<const char *> (optval_),
                                      optvallen_);
                as_server = 0;
                mechanism = SZMQ_PLAIN;
                return 0;
            }
            break;

        case SZMQ_PLAIN_PASSWORD:
            if (optvallen_ == 0 && optval_ == NULL) {
                mechanism = SZMQ_NULL;
                return 0;
            } else if (optvallen_ > 0 && optvallen_ <= UCHAR_MAX
...
#ifdef SZMQ_HAVE_CURVE
            case SZMQ_CURVE_SERVER:
                if (is_int && (value == 0 || value == 1)) {
                    as_server = value;
                    mechanism = value ? SZMQ_CURVE : SZMQ_NULL;
                    return 0;
                }
                break;

            case SZMQ_CURVE_PUBLICKEY:
                if (0 == set_curve_key (curve_public_key, optval_, optvallen_)) {
                    return 0;
                }
                break;

            case SZMQ_CURVE_SECRETKEY:
                if (0 == set_curve_key (curve_secret_key, optval_, optvallen_)) {
                    return 0;
                }
                break;

            case SZMQ_CURVE_SERVERKEY:
        ...
        case SZMQ_CONFLATE:
            ...
            case SZMQ_GSSAPI_SERVER:
                if (is_int && (value == 0 || value == 1)) {
                    as_server = value;
                    mechanism = SZMQ_GSSAPI;
                    return 0;
                }
                break;

            case SZMQ_GSSAPI_PRINCIPAL:
            ...
            case SZMQ_GSSAPI_SERVICE_PRINCIPAL:
                if (optvallen_ > 0 && optvallen_ <= UCHAR_MAX && optval_ != NULL) {
            ...
            case SZMQ_GSSAPI_PLAINTEXT:
                return do_setsockopt_int_as_bool_strict (optval_, optvallen_,
                                                         &gss_plaintext);

            case SZMQ_GSSAPI_PRINCIPAL_NAMETYPE:
                if (is_int
                    && (value == SZMQ_GSSAPI_NT_HOSTBASED
                        || value == SZMQ_GSSAPI_NT_USER_NAME
                        || value == SZMQ_GSSAPI_NT_KRB5_PRINCIPAL)) {
                    gss_principal_nt = value;
                    return 0;
                }
                break;

            case SZMQ_GSSAPI_SERVICE_PRINCIPAL_NAMETYPE:
                if (is_int
                    && (value == SZMQ_GSSAPI_NT_HOSTBASED
                        || value == SZMQ_GSSAPI_NT_USER_NAME
                        || value == SZMQ_GSSAPI_NT_KRB5_PRINCIPAL)) {
        ...
        case SZMQ_BINDTODEVICE:
            return do_setsockopt_string_allow_empty_strict(
                    optval_, optvallen_, &bound_device, BINDDEVSIZ);

        case SZMQ_ZAP_ENFORCE_DOMAIN:
            return do_setsockopt_int_as_bool_relaxed(optval_, optvallen_,
                                                     &zap_enforce_domain);

        case SZMQ_LOOPBACK_FASTPATH:
            return do_setsockopt_int_as_bool_relaxed(optval_, optvallen_,
                                                     &loopback_fastpath);
        ...
        case SZMQ_MULTICAST_LOOP:
            return do_setsockopt_int_as_bool_relaxed(optval_, optvallen_,
                                                     &multicast_loop);

#ifdef SZMQ_BUILD_DRAFT_API
            case SZMQ_IN_BATCH_SIZE:
                if (is_int && value > 0) {
                    in_batch_size = value;
                    return 0;
                }
        ...
        default:
            ...
            //  socket option will, if SZMQ_ACT_MILITANT is defined.
            malformed = false;
#endif
            break;
    }
...
int zmq::options_t::getsockopt(int option_,
    ...
    switch (option_) {
        ...
        case SZMQ_PLAIN_SERVER:
            if (is_int) {
                *value = as_server && mechanism == SZMQ_PLAIN;
                return 0;
            }
            break;
        ...
        case SZMQ_ZAP_DOMAIN:
            ...
            case SZMQ_CURVE_SERVER:
                if (is_int) {
                    *value = as_server && mechanism == SZMQ_CURVE;
                    return 0;
                }
                break;
        ...
        case SZMQ_CONFLATE:
            ...
            case SZMQ_GSSAPI_SERVER:
                if (is_int) {
                    *value = as_server && mechanism == SZMQ_GSSAPI;
                    return 0;
                }
                break;
