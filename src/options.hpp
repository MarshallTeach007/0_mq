...
#ifndef __SZMQ_OPTIONS_HPP_INCLUDED__
#define __SZMQ_OPTIONS_HPP_INCLUDED__
...
#include <sys/types.h>
#endif
#ifdef SZMQ_HAVE_LOCAL_PEERCRED
#include <sys/ucred.h>
#endif
...
namespace zmq {
    struct options_t {
        ...
        // Router socket SZMQ_NOTIFY_CONNECT/SZMQ_NOTIFY_DISCONNECT notifications
        int router_notify;
    ...
    inline bool get_effective_conflate_option(const options_t &options) {
        ...
        return options.conflate
               && (options.type == SZMQ_DEALER || options.type == SZMQ_PULL
                   || options.type == SZMQ_PUSH || options.type == SZMQ_PUB
                   || options.type == SZMQ_SUB);
    }
