...
void zmq::mechanism_t::set_user_id(const void *user_id_, size_t size_) {
    ...
    _zap_properties.SZMQ_MAP_INSERT_OR_EMPLACE (
            std::string(SZMQ_MSG_PROPERTY_USER_ID),
...
const char *zmq::mechanism_t::socket_type_string(int socket_type_) {
    // TODO the order must of the names must correspond to the values resp. order of SZMQ_* socket type definitions in zmq.h!
...
size_t zmq::mechanism_t::add_basic_properties(unsigned char *ptr_,
    ...
    if (options.type == SZMQ_REQ || options.type == SZMQ_DEALER
        || options.type == SZMQ_ROUTER) {
...
size_t zmq::mechanism_t::basic_properties_len() const {
    ...
    return property_len(ZMTP_PROPERTY_SOCKET_TYPE, strlen(socket_type))
           + meta_len
           + ((options.type == SZMQ_REQ || options.type == SZMQ_DEALER
               || options.type == SZMQ_ROUTER)
