...
#ifdef SZMQ_USE_GNUTLS
#include <gnutls/gnutls.h>
#endif

#define SZMQ_CTX_TAG_VALUE_GOOD 0xabadcafe
#define SZMQ_CTX_TAG_VALUE_BAD 0xdeadbeef
...
zmq::ctx_t::ctx_t() :
        _tag(SZMQ_CTX_TAG_VALUE_GOOD),
        _starting(true),
        _terminating(false),
        _reaper(NULL),
        _max_sockets(clipped_maxsocket(SZMQ_MAX_SOCKETS_DFLT)),
        _max_msgsz(INT_MAX),
        _io_thread_count(SZMQ_IO_THREADS_DFLT),
        _blocky(true),
        _ipv6(false),
        _zero_copy(true) {
#ifdef HAVE_FORK
...
bool zmq::ctx_t::check_tag() const {
    return _tag == SZMQ_CTX_TAG_VALUE_GOOD;
}
...
zmq::ctx_t::~ctx_t() {
    ...
    for (io_threads_t::size_type i = 0; i != io_threads_size; i++) {
        LIBSZMQ_DELETE (_io_threads[i]);
    }

    //  Deallocate the reaper thread object.
    LIBSZMQ_DELETE (_reaper);
...
int zmq::ctx_t::terminate() {
    ...
    for (pending_connections_t::iterator p = copy.begin(), end = copy.end();
         p != end; ++p) {
        zmq::socket_base_t *s = create_socket(SZMQ_PAIR);
...
zmq::thread_ctx_t::thread_ctx_t() :
        _thread_priority(SZMQ_THREAD_PRIORITY_DFLT),
        _thread_sched_policy(SZMQ_THREAD_SCHED_POLICY_DFLT) {
}
...
int zmq::thread_ctx_t::set(int option_, const void *optval_, size_t optvallen_) {
    ...
    switch (option_) {
        ...
        case SZMQ_THREAD_AFFINITY_CPU_ADD:
            if (is_int && value >= 0) {
                scoped_lock_t locker(_opt_sync);
        ...
        case SZMQ_THREAD_AFFINITY_CPU_REMOVE:
            if (is_int && value >= 0) {
                scoped_lock_t locker(_opt_sync);
...
int zmq::ctx_t::register_endpoint(const char *addr_,
    ...
    const bool inserted =
            _endpoints.SZMQ_MAP_INSERT_OR_EMPLACE (std::string(addr_), endpoint_)
                    .second;
    if (!inserted) {
        errno = EADDRINUSE;
        return -1;
...
void zmq::ctx_t::pend_connection(const std::string &addr_,
    ...
    if (it == _endpoints.end()) {
        ...
        _pending_connections.SZMQ_MAP_INSERT_OR_EMPLACE (addr_,
                                                         pending_connection);
    } else {
