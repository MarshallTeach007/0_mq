...
int zmq::tune_tcp_keepalives(fd_t s_,
    ...
    LIBSZMQ_UNUSED (keepalive_);
    LIBSZMQ_UNUSED (keepalive_cnt_);
    LIBSZMQ_UNUSED (keepalive_idle_);
    LIBSZMQ_UNUSED (keepalive_intvl_);
...
#endif // SZMQ_HAVE_TCP_KEEPCNT

#ifdef SZMQ_HAVE_TCP_KEEPIDLE
...
#else // SZMQ_HAVE_TCP_KEEPIDLE
#ifdef SZMQ_HAVE_TCP_KEEPALIVE
        if (keepalive_idle_ != -1) {
...
#endif // SZMQ_HAVE_TCP_KEEPALIVE
#endif // SZMQ_HAVE_TCP_KEEPIDLE

#ifdef SZMQ_HAVE_TCP_KEEPINTVL
...
#endif // SZMQ_HAVE_TCP_KEEPINTVL
    }
#endif // SZMQ_HAVE_SO_KEEPALIVE
#endif // SZMQ_HAVE_WINDOWS
...
int zmq::tune_tcp_maxrt(fd_t sockfd_, int timeout_) {
    ...
    LIBSZMQ_UNUSED (sockfd_);

#if defined(SZMQ_HAVE_WINDOWS) && defined(TCP_MAXRT)
    ...
    assert_success_or_recoverable (sockfd_, rc);
    return rc;
// FIXME: should be SZMQ_HAVE_TCP_USER_TIMEOUT
...
#if defined SZMQ_HAVE_WINDOWS && defined SIO_LOOPBACK_FAST_PATH
