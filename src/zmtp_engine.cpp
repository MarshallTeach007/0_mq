    ...
    if (_greeting_bytes_read > signature_size) {
        if (_outpos + _outsize == _greeting_send + signature_size + 1) {
            ...
            if (_greeting_recv[revision_pos] == ZMTP_1_0
                ...
                zmq_assert (_options.mechanism == SZMQ_NULL
                            || _options.mechanism == SZMQ_PLAIN
                            || _options.mechanism == SZMQ_CURVE
                            || _options.mechanism == SZMQ_GSSAPI);

                if (_options.mechanism == SZMQ_NULL)
                    memcpy(_outpos + _outsize, "NULL", 4);
                else if (_options.mechanism == SZMQ_PLAIN)
                    memcpy(_outpos + _outsize, "PLAIN", 5);
                else if (_options.mechanism == SZMQ_GSSAPI)
                    memcpy(_outpos + _outsize, "GSSAPI", 6);
                else if (_options.mechanism == SZMQ_CURVE)
...
bool zmq::zmtp_engine_t::handshake_v1_0_unversioned() {
    ...
    if (_options.type == SZMQ_PUB || _options.type == SZMQ_XPUB)
