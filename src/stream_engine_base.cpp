...
#elif defined SZMQ_HAVE_LOCAL_PEERCRED
    else if (family == PF_UNIX) {
        struct xucred cred;
        socklen_t size = sizeof (cred);
...
zmq::stream_engine_base_t::~stream_engine_base_t() {
    ...
    if (_metadata != NULL) {
        if (_metadata->drop_ref()) {
            LIBSZMQ_DELETE (_metadata);
        }
    }

    LIBSZMQ_DELETE (_encoder);
    LIBSZMQ_DELETE (_decoder);
    LIBSZMQ_DELETE (_mechanism);
}
...
void zmq::stream_engine_base_t::mechanism_ready() {
    ...
    if (_options.router_notify & SZMQ_NOTIFY_CONNECT) {
...
void zmq::stream_engine_base_t::error(error_reason_t reason_) {
    ...
    if ((_options.router_notify & SZMQ_NOTIFY_DISCONNECT) && !_handshaking) {
    ...
    if (reason_ != protocol_error
        ...
        if (((reason_ == connection_error) || (reason_ == timeout_error))
            && (_options.reconnect_stop
                & SZMQ_RECONNECT_STOP_HANDSHAKE_FAILED)) {
...
bool zmq::stream_engine_base_t::init_properties(properties_t &properties_) {
    if (_peer_address.empty())
        return false;
    properties_.SZMQ_MAP_INSERT_OR_EMPLACE (
            std::string(SZMQ_MSG_PROPERTY_PEER_ADDRESS), _peer_address);

    //  Private property to support deprecated SRCFD
    std::ostringstream stream;
    stream << static_cast<int> (_s);
    std::string fd_string = stream.str();
    properties_.SZMQ_MAP_INSERT_OR_EMPLACE (std::string("__fd"),
                                            SZMQ_MOVE(fd_string));
    return true;
}
