...
void zmq::radio_t::xread_activated(pipe_t *pipe_) {
    ...
    while (pipe_->read(&msg)) {
        ...
        if (msg.is_join() || msg.is_leave()) {
            ...
            if (msg.is_join())
                _subscriptions.SZMQ_MAP_INSERT_OR_EMPLACE (SZMQ_MOVE(group),
                                                           pipe_);
            else {
...
int zmq::radio_t::xsend(msg_t *msg_) {
    //  Radio sockets do not allow multipart data (SZMQ_SNDMORE)
