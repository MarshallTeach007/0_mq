...
#define SZMQ_HAVE_CRYPTO_BOX_EASY_FNS 1
#endif
#endif
...
int zmq::curve_encoding_t::check_validity (msg_t *msg_, int *error_event_code_)
{
    ...
    if (size < message_command_len
        ...
        *error_event_code_ = SZMQ_PROTOCOL_ERROR_ZMTP_UNEXPECTED_COMMAND;
        errno = EPROTO;
        return -1;
    }

    if (size < message_header_len + crypto_box_MACBYTES + flags_len) {
        *error_event_code_ = SZMQ_PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_MESSAGE;
        errno = EPROTO;
        return -1;
    }

    {
        ...
        if (nonce <= _cn_peer_nonce) {
            *error_event_code_ = SZMQ_PROTOCOL_ERROR_ZMTP_INVALID_SEQUENCE;
            errno = EPROTO;
            return -1;
        }
...
#ifdef SZMQ_HAVE_CRYPTO_BOX_EASY_FNS
...
int zmq::curve_encoding_t::decode (msg_t *msg_, int *error_event_code_)
{
    ...
    if (rc == 0) {
        ...
        *error_event_code_ = SZMQ_PROTOCOL_ERROR_ZMTP_CRYPTOGRAPHIC;
        errno = EPROTO;
    }
