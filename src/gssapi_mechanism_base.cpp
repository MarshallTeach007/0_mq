...
int zmq::gssapi_mechanism_base_t::decode_message (msg_t *msg_)
{
    ...
    if (bytes_left < 8 || memcmp (ptr, "\x07MESSAGE", 8)) {
        session->get_socket ()->event_handshake_failed_protocol (
          session->get_endpoint (), SZMQ_PROTOCOL_ERROR_ZMTP_UNEXPECTED_COMMAND);
        errno = EPROTO;
        return -1;
    }
    ptr += 8;
    ...
    if (bytes_left < 4) {
        session->get_socket ()->event_handshake_failed_protocol (
          ...
          SZMQ_PROTOCOL_ERROR_ZMTP_MALFORMED_COMMAND_MESSAGE);
        errno = EPROTO;
        return -1;
    }
    gss_buffer_desc wrapped;
    ...
    if (maj_stat != GSS_S_COMPLETE) {
        ...
        free (wrapped.value);
        session->get_socket ()->event_handshake_failed_protocol (
          session->get_endpoint (), SZMQ_PROTOCOL_ERROR_ZMTP_CRYPTOGRAPHIC);
        errno = EPROTO;
        return -1;
    }
    zmq_assert (state);
...
int zmq::gssapi_mechanism_base_t::process_ready (msg_t *msg_)
{
    ...
    if (rc == -1)
        session->get_socket ()->event_handshake_failed_protocol (
          session->get_endpoint (), SZMQ_PROTOCOL_ERROR_ZMTP_INVALID_METADATA);
...
{
    switch (zmq_nametype) {
        case SZMQ_GSSAPI_NT_HOSTBASED:
            return GSS_C_NT_HOSTBASED_SERVICE;
        case SZMQ_GSSAPI_NT_USER_NAME:
            return GSS_C_NT_USER_NAME;
        case SZMQ_GSSAPI_NT_KRB5_PRINCIPAL:
