...
,
_pollfds (NULL)
#elif defined SZMQ_POLL_BASED_ON_SELECT
,
_max_fd (0)
#endif
{
            ...
            if (it->socket) {
                if (!is_thread_safe (*it->socket)) {
                    ...
                    const int rc = it->socket->getsockopt (
                      SZMQ_FD, &_pollfds[item_nbr].fd, &fd_size);
                    zmq_assert (rc == 0);
                ...
                _pollfds[item_nbr].events =
                  (it->events & SZMQ_POLLIN ? POLLIN : 0)
                  | (it->events & SZMQ_POLLOUT ? POLLOUT : 0)
                  | (it->events & SZMQ_POLLPRI ? POLLPRI : 0);
        ...
        if (it->events) {
            ...
            if (it->socket) {
                if (!is_thread_safe (*it->socket)) {
                    ...
                    size_t fd_size = sizeof (zmq::fd_t);
                    int rc =
                      it->socket->getsockopt (SZMQ_FD, &notify_fd, &fd_size);
                    zmq_assert (rc == 0);
            ...
            //  events to the appropriate fd_sets.
                ...
                if (it->events & SZMQ_POLLIN)
                    FD_SET (it->fd, _pollset_in.get ());
                if (it->events & SZMQ_POLLOUT)
                    FD_SET (it->fd, _pollset_out.get ());
                if (it->events & SZMQ_POLLERR)
...
int found = 0;
for (
items_t::iterator it = _items.begin(), end = _items.end();
it !=
end &&found<n_events_;
++it) {
...
uint32_t events;
if (it->socket->getsockopt (SZMQ_EVENTS, &events, &events_size)
== -1) {
return -1;
}
...
int zmq::socket_poller_t::wait(zmq::socket_poller_t::event_t *events_,
    ...
    if (unlikely (_pollset_size == 0)) {
        ...
        return -1;
#elif defined SZMQ_HAVE_ANDROID
        usleep (timeout_ * 1000);
        return -1;
#elif defined SZMQ_HAVE_OSX
        usleep (timeout_ * 1000);
        errno = EAGAIN;
        return -1;
