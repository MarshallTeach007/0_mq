...
#ifndef __SZMQ_DECODER_HPP_INCLUDED__
#define __SZMQ_DECODER_HPP_INCLUDED__
...
//  Buffer management is done by an allocator policy.
    ...
    public:
        ...
        ~decoder_base_t() SZMQ_OVERRIDE { _allocator.deallocate(); }

        //  Returns a buffer to be filled with binary data.
        void get_buffer(unsigned char **data_, std::size_t *size_) SZMQ_FINAL
        {
            _buf = _allocator.allocate();
        ...
        int decode(const unsigned char *data_,
                   std::size_t size_,
                   std::size_t &bytes_used_) SZMQ_FINAL
        {
            bytes_used_ = 0;
        ...
        void resize_buffer(std::size_t new_size_) SZMQ_FINAL
        {
    ...
    SZMQ_NON_COPYABLE_NOR_MOVABLE (decoder_base_t)
    };
}
