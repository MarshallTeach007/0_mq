...
#ifndef __SZMQ_NORM_ENGINE_HPP_INCLUDED__
#define __SZMQ_NORM_ENGINE_HPP_INCLUDED__

#if defined SZMQ_HAVE_NORM

#if defined(SZMQ_HAVE_WINDOWS) && defined(SZMQ_IOTHREAD_POLLER_USE_EPOLL)
#define SZMQ_USE_NORM_SOCKET_WRAPPER
#endif
...
class norm_engine_t SZMQ_FINAL : public io_object_t, public i_engine
{
  public:
    norm_engine_t (zmq::io_thread_t *parent_, const options_t &options_);
    ~norm_engine_t () SZMQ_FINAL;

    // create NORM instance, session, etc
    int init (const char *network_, bool send, bool recv);
    void shutdown ();

    bool has_handshake_stage () SZMQ_FINAL { return false; };

    //  i_engine interface implementation.
    //  Plug the engine to the session.
    void plug (zmq::io_thread_t *io_thread_,
               class session_base_t *session_) SZMQ_FINAL;

    //  Terminate and deallocate the engine. Note that 'detached'
    //  events are not fired on termination.
    void terminate () SZMQ_FINAL;

    //  This method is called by the session to signalise that more
    //  messages can be written to the pipe.
    bool restart_input () SZMQ_FINAL;

    //  This method is called by the session to signalise that there
    //  are messages to send available.
    void restart_output () SZMQ_FINAL;

    void zap_msg_available () SZMQ_FINAL {}

    const endpoint_uri_pair_t &get_endpoint () const SZMQ_FINAL;
...
#ifdef SZMQ_USE_NORM_SOCKET_WRAPPER
    fd_t
...
#endif // SZMQ_HAVE_NORM

#endif // !__SZMQ_NORM_ENGINE_HPP_INCLUDED__
