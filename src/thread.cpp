...
{
    // not implemented
    LIBSZMQ_UNUSED (priority_);
    LIBSZMQ_UNUSED (scheduling_policy_);
    LIBSZMQ_UNUSED (affinity_cpus_);
}
...
extern "C" {
static void *thread_routine(void *arg_) {
#if !defined SZMQ_HAVE_OPENVMS && !defined SZMQ_HAVE_ANDROID
...
applySchedulingParameters() // to be called in secondary thread context
{
  ...
  && defined _SC_THREAD_PRIORITY_SCHEDULING
    ...
    if (_thread_sched_policy != SZMQ_THREAD_SCHED_POLICY_DFLT) {
    ...
    */
    ...
    if (_thread_priority != SZMQ_THREAD_PRIORITY_DFLT) {
...
#ifdef SZMQ_HAVE_PTHREAD_SET_AFFINITY
...
#if defined(SZMQ_HAVE_ANDROID)
    return;
#endif

#if defined(SZMQ_HAVE_PTHREAD_SETNAME_1)
    int rc = pthread_setname_np (_name);
    if (rc)
        return;
#elif defined(SZMQ_HAVE_PTHREAD_SETNAME_2)
    int rc = pthread_setname_np (pthread_self (), _name);
    if (rc)
        return;
#elif defined(SZMQ_HAVE_PTHREAD_SETNAME_3)
    int rc = pthread_setname_np (pthread_self (), _name, NULL);
    if (rc)
        return;
#elif defined(SZMQ_HAVE_PTHREAD_SET_NAME)
