...
#elif defined SZMQ_USE_BUILTIN_SHA1
#include "../external/sha1/sha1.h"
#elif defined SZMQ_USE_GNUTLS
#define SHA_DIGEST_LENGTH 20
...
bool zmq::ws_engine_t::select_protocol(const char *protocol_) {
    if (_options.mechanism == SZMQ_NULL && (strcmp("ZWS2.0", protocol_) == 0)) {
    ...
    while (_insize > 0) {
        ...
        if (_server_handshake_state == handshake_error) {
            ...
            socket()->event_handshake_failed_protocol(
                    _endpoint_uri_pair, SZMQ_PROTOCOL_ERROR_WS_UNSPECIFIED);
