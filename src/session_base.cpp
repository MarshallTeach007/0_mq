...
int zmq::session_base_t::zap_connect() {
    ...
    zmq_assert (peer.options.type == SZMQ_REP || peer.options.type == SZMQ_ROUTER
                || peer.options.type == SZMQ_SERVER);
...
bool zmq::session_base_t::zap_enabled() const {
    return (options.mechanism != SZMQ_NULL || !options.zap_domain.empty());
}
...
void zmq::session_base_t::reconnect() {
    ...
    if (_pipe
        && (options.type == SZMQ_SUB || options.type == SZMQ_XSUB
            || options.type == SZMQ_DISH))
        _pipe->hiccup();
}
...
void zmq::session_base_t::start_connecting(bool wait_) {
    ...
    if (_addr->protocol == protocol_name::udp) {
        zmq_assert (options.type == SZMQ_DISH || options.type == SZMQ_RADIO
        ...
        if (options.type == SZMQ_RADIO) {
            send = true;
            recv = false;
        } else if (options.type == SZMQ_DISH) {
            send = false;
            recv = true;
        } else if (options.type == SZMQ_DGRAM) {
            send = true;
            recv = true;
        }
        ...
        if (_addr->protocol == "pgm" || _addr->protocol == "epgm") {
            zmq_assert (options.type == SZMQ_PUB || options.type == SZMQ_XPUB
                        || options.type == SZMQ_SUB || options.type == SZMQ_XSUB);
            ...
            if (options.type == SZMQ_PUB || options.type == SZMQ_XPUB) {
                //  PGM sender.
